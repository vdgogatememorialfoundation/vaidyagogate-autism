...
function findUserByEmail(db, email, cb) {
    ...
    db.get(
        `SELECT id, user_id_string, first_name, last_name, email, phone, password, role, user_role,
                ...
                doctor_category, doctor_modules, admin_modules,
...
function sqlPhoneLast10Equals(column, paramPlaceholder) {
    const digits = `REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(${column}, ''), ' ', ''), '-', ''), '+', ''), '.', '')`;
    return `SUBSTR(${digits}, -10) = ${paramPlaceholder}`;
}
...
function findUserByPhone(db, phone, cb) {
    ...
    db.get(
        `SELECT id, user_id_string, first_name, middle_name, last_name, email, phone, password, role, user_role,
                ...
                COALESCE(is_demo, 0) AS is_demo, doctor_category, doctor_modules, admin_modules,
                COALESCE(email_verified, 1) AS email_verified, created_at, activated_at, last_login_at
         FROM users
         WHERE ${sqlUserActive('is_disabled')} AND ${sqlPhoneLast10Equals('phone', '?')}`,
        [norm],
        cb
    );
}
