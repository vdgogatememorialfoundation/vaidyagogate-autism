...
const SEMINAR_TRACK_STEPS = [
    ...
    { key: 'pending_approval', title: 'Under review', icon: 'fa-user-shield' },
...
const CASE_TRACK_STEPS = [
    ...
    { key: 'under_review', title: 'Under review', icon: 'fa-user-shield' },
...
const PREREG_TRACK_STEPS = [
    { key: 'submitted', title: 'Application submitted', icon: 'fa-clipboard-check' },
    { key: 'under_review', title: 'Under review', icon: 'fa-magnifying-glass' },
    { key: 'approved', title: 'Pre-registration approved', icon: 'fa-circle-check' },
    { key: 'main_registration', title: 'Main registration', icon: 'fa-file-signature' },
    { key: 'e_ticket', title: 'E-ticket', icon: 'fa-qrcode' }
];

function generateNumericTrackingId(digits) {
    let id = '';
    const n = Number.isInteger(digits) && digits > 0 ? digits : 12;
    for (let i = 0; i < n; i++) id += Math.floor(Math.random() * 10).toString();
    return id;
}

function generatePreregTrackingId() {
    return generateNumericTrackingId(12);
}

function generateMainRegTrackingId() {
    return generateNumericTrackingId(12);
}
                            ...
                            (e3) => {
                                ...
                                db.run(
                                    ...
                                    (e4) => {
                                        ie(e4);
                                        db.run(
                                            `CREATE TABLE IF NOT EXISTS preregistration_status_log (
                                                id INTEGER PRIMARY KEY AUTOINCREMENT,
                                                preregistration_id INTEGER NOT NULL,
                                                step_key TEXT NOT NULL,
                                                label TEXT,
                                                message TEXT,
                                                created_at DATETIME DEFAULT CURRENT_TIMESTAMP
                                            ...
                                            (e5) => {
                                                ie(e5);
                                                db.run(
                                                    `CREATE INDEX IF NOT EXISTS idx_prereg_status_log ON preregistration_status_log(preregistration_id, created_at)`,
                                                    (e6) => {
                                                        ie(e6);
                                                ...
                                                (eB) => {
                                                    ...
                                                    db.run(
                                                        ...
                                                        (eB2) => {
                                                            ignoreErr(eB2);
                                                            db.run(
                                                                `INSERT INTO preregistration_status_log (preregistration_id, step_key, label, message, created_at)
                                                                 SELECT p.id, 'submitted', 'Application submitted', 'Pre-registration received.', p.created_at
                                                                 FROM preregistrations p
                                                                 WHERE NOT EXISTS (
                                                                    SELECT 1 FROM preregistration_status_log l
                                                                    WHERE l.preregistration_id = p.id AND l.step_key = 'submitted'
                                                                 )`,
                                                                (eB3) => {
                                                                    ignoreErr(eB3);
                                                                    if (next) next();
    ...
    const steps = stepDefs.map((def) => {
        ...
        if (status === 'rejected' || status === 'cancelled') {
            ...
            const upcomingDesc = noFees
                ? {
                      approved: 'Approval after review.',
...
function buildCaseTimeline(submission, logs) {
    ...
    const steps = CASE_TRACK_STEPS.map((def) => {
        ...
        if (def.key === 'submitted') {
            ...
            desc = log && log.message ? log.message : 'Your uploaded files are under review.';
        ...
        db.get(
            ...
            (eT, tRow) => {
                db.get(
                    ...
                    (eSc, scanRow) => {
                        db.get(
                            `SELECT uc.enabled, uc.scan_verified, ct.file_path AS template_path
                             FROM user_certificates uc
                             LEFT JOIN certificate_templates ct ON ct.id = uc.template_id AND COALESCE(ct.is_active, 1) = 1
                             WHERE uc.registration_id = ? LIMIT 1`,
                            [row.id],
                            (eC, certRow) => {
...
function registrationStatusToLog(status, prevStatus, noFees) {
    ...
    const map = {
        ...
        e_ticket_issued: {
            ...
            message: 'E-ticket is available in the applicant portal.'
        },
        certificate_issued: {
            key: 'certificate',
...
function caseStatusToLog(status, extra) {
    ...
    const map = {
        ...
        under_review: { key: 'under_review', label: 'Under review', message: 'Your uploaded files are under review.' },
...
function logPreregistrationEvent(db, preregistrationId, stepKey, label, message, cb) {
    if (!preregistrationId || !stepKey) {
        if (cb) cb();
        return;
    }
    db.run(
        `INSERT INTO preregistration_status_log (preregistration_id, step_key, label, message) VALUES (?, ?, ?, ?)`,
        [preregistrationId, stepKey, label || null, message || null],
        (e) => {
            if (e && /relation .* does not exist/i.test(String(e.message))) {
                if (cb) return cb(null);
            }
...
function fetchPreregistrationLogs(db, preregistrationId, cb) {
    db.all(
        `SELECT step_key, label, message, created_at FROM preregistration_status_log
         WHERE preregistration_id = ? ORDER BY created_at ASC`,
        [preregistrationId],
        (e, rows) => {
            if (e && /does not exist|relation/i.test(String(e.message))) return cb(null, []);
            cb(e, rows);
...
function preregStatusToLog(newStatus, note) {
    ...
    if (st === 'approved') {
        return [
            {
                key: 'approved',
                label: 'Pre-registration approved',
                message: msg || 'You can proceed to main registration when it opens.'
            }
        ];
    }
    if (st === 'rejected') {
        return [{ key: 'rejected', label: 'Not approved', message: msg || 'Pre-registration was not approved.' }];
    }
        ...
        return [
            {
                ...
                message: msg || 'Please update and resubmit your pre-registration.'
            }
        ];
    }
    if (st === 'submitted') {
        return [{ key: 'submitted', label: 'Application submitted', message: msg || 'Pre-registration received.' }];
    }
    return [{ key: 'under_review', label: 'Under review', message: msg || 'Status updated.' }];
}

function buildPreregTimeline(prereg, logs, extra) {
    const status = String((prereg && prereg.status) || 'submitted').toLowerCase();
    const logByKey = {};
    (logs || []).forEach((l) => {
        if (!logByKey[l.step_key]) logByKey[l.step_key] = l;
    });
    const submittedAt = (logByKey.submitted && logByKey.submitted.created_at) || prereg.created_at;
    const hasReg = !!(extra && extra.registrationId);
    const regSt = String((extra && extra.registrationStatus) || '').toLowerCase();
    const rejected = status === 'rejected';
    const revision = status === 'revision_required';

    const steps = PREREG_TRACK_STEPS.map((def) => {
        let state = 'pending';
        let at = null;
        let desc = '';
        const log = logByKey[def.key];
        ...
        if (def.key === 'submitted') {
            state = 'completed';
            at = submittedAt;
            desc = (log && log.message) || 'Your pre-registration was received.';
        } else if (def.key === 'under_review') {
            desc = (log && log.message) || 'Our team is checking your details.';
            if (rejected) {
                state = 'fail';
                at = (logByKey.rejected && logByKey.rejected.created_at) || prereg.updated_at || null;
                desc = (logByKey.rejected && logByKey.rejected.message) || 'Not approved at this stage.';
            } else if (revision) {
                state = 'active';
                at = (logByKey.revision_required && logByKey.revision_required.created_at) || prereg.updated_at || null;
                desc = (logByKey.revision_required && logByKey.revision_required.message) || 'Please update and resubmit.';
            } else if (status === 'submitted') {
                state = 'active';
                at = (log && log.created_at) || submittedAt;
            } else if (status === 'approved' || hasReg) {
                state = 'completed';
                at =
                    (log && log.created_at) ||
                    (logByKey.under_review && logByKey.under_review.created_at) ||
                    prereg.updated_at ||
                    null;
            }
        } else if (def.key === 'approved') {
            desc = (log && log.message) || 'You can proceed to main registration when it opens.';
            if (status === 'approved' || hasReg) {
                state = 'completed';
                at = (logByKey.approved && logByKey.approved.created_at) || prereg.updated_at || null;
            }
        } else if (def.key === 'main_registration') {
            desc = hasReg ? 'Final registration submitted or in progress.' : 'Opens after pre-registration approval.';
            if (hasReg) {
                const regDone =
                    regSt === 'e_ticket_issued' || regSt === 'checked_in' || regSt === 'completed' || regSt === 'approved';
                state = regDone ? 'completed' : 'active';
                at = (logByKey.main_registration && logByKey.main_registration.created_at) || null;
            }
        } else if (def.key === 'e_ticket') {
            desc = (log && log.message) || 'Download your pass with QR code for event day.';
            const ticketReady = regSt === 'e_ticket_issued' || regSt === 'checked_in' || regSt === 'completed';
            if (ticketReady) {
                state = 'completed';
                at = (logByKey.e_ticket && logByKey.e_ticket.created_at) || null;
            } else if (hasReg && (status === 'approved' || regSt === 'submitted' || regSt === 'approved')) {
                state = 'active';
            }
        }
...
function attachPreregistrationTimelines(db, rows, cb) {
    const list = rows || [];
    if (!list.length) return cb(null, list);
    let pending = list.length;
    const out = list.map((r) => ({ ...r }));
    out.forEach((row, idx) => {
        fetchPreregistrationLogs(db, row.id, (eL, logs) => {
            if (eL) {
                out[idx].timeline = buildPreregTimeline(row, [], {
                    registrationId: row.registration_id,
                    registrationStatus: row.registration_status
                });
            } else {
                out[idx].timeline = buildPreregTimeline(row, logs, {
                    registrationId: row.registration_id,
                    registrationStatus: row.registration_status
                });
            }
            out[idx].trackingKind = 'preregistration';
            pending -= 1;
            if (pending === 0) cb(null, out);
        });
    });
...
module.exports = {
    ...
    attachCaseTimelines,
    attachPreregistrationTimelines,
    registrationStatusToLog,
    caseStatusToLog,
    preregStatusToLog,
    logPreregistrationEvent,
    fetchPreregistrationLogs,
    buildPreregTimeline,
    PREREG_TRACK_STEPS,
    generatePreregTrackingId,
    generateMainRegTrackingId,
    generateNumericTrackingId,
