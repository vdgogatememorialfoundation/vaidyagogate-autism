...
function generatePrefixedTrackingId(prefix, digits) {
    let id = String(prefix || '');
    const n = Number.isInteger(digits) && digits > 0 ? digits : 10;
    for (let i = 0; i < n; i++) id += Math.floor(Math.random() * 10).toString();
    return id;
}

function generatePreregTrackingId() {
    return generatePrefixedTrackingId('PRE', 10);
}

function generateMainRegTrackingId() {
    return generatePrefixedTrackingId('REG', 10);
}
