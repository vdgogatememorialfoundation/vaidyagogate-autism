...
function consumeAllPhoneAuthOtps(db, channel, destination, cb) {
    const dest = normalizeOtpDestination(channel, destination);
    db.run(
        `DELETE FROM otp_whatsapp_deliveries WHERE otp_code_id IN (
            SELECT id FROM otp_codes WHERE channel = ? AND destination = ?
            AND (purpose = 'signup' OR purpose LIKE 'login:%' OR purpose LIKE 'applicant:%')
        )`,
        [channel, dest],
        () => {
            db.run(
...
function ensureOtpWhatsAppDeliverySchema(db, cb) {
...
/** DB-backed claim — prevents duplicate WhatsApp when parallel requests hit different server instances. */
function claimOtpWhatsAppDelivery(db, otpRowId, forceResend, cb) {
    if (!db || !otpRowId) return cb(null, true);
    ensureOtpWhatsAppDeliverySchema(db, (schemaErr) => {
        if (schemaErr) return cb(schemaErr);
        const insert = () => {
            db.run(`INSERT INTO otp_whatsapp_deliveries (otp_code_id) VALUES (?)`, [otpRowId], (insErr) => {
                if (isUniqueViolation(insErr)) return cb(null, false);
                if (insErr) return cb(insErr);
                cb(null, true);
            });
        };
        if (forceResend) {
            return db.run(`DELETE FROM otp_whatsapp_deliveries WHERE otp_code_id = ?`, [otpRowId], (delErr) => {
...
function releaseOtpWhatsAppDelivery(db, otpRowId, cb) {
    if (!db || !otpRowId) {
        if (cb) cb(null);
        return;
    }
    db.run(`DELETE FROM otp_whatsapp_deliveries WHERE otp_code_id = ?`, [otpRowId], cb || (() => {}));
}
...
function isPhoneAuthPurpose(purpose) {
    return purpose === 'signup' || purpose === 'login';
}

function metaForStore(purpose, meta, code) {
    const base = meta && typeof meta === 'object' ? Object.assign({}, meta) : {};
    if (isPhoneAuthPurpose(purpose)) base._otp = String(code);
    return JSON.stringify(base);
}

function readOtpFromMeta(metaStr) {
    if (!metaStr) return null;
    try {
        const m = JSON.parse(metaStr);
        return m && m._otp != null ? String(m._otp) : null;
    } catch (_) {
        return null;
    }
}

function invalidateLegacyApplicantOtps(db, channel, destination, cb) {
    const dest = normalizeOtpDestination(channel, destination);
    db.run(
        `UPDATE otp_codes SET consumed = 1 WHERE channel = ? AND destination = ? AND consumed = 0 AND purpose LIKE 'applicant:%'`,
        [channel, dest],
        cb || (() => {})
    );
}

function getActiveAuthOtpCode(db, { channel, destination, purpose, meta }, cb) {
    const dest = normalizeOtpDestination(channel, destination);
    const pk = otpRowPurpose(purpose, meta);
    const now = new Date().toISOString();
    db.get(
        `SELECT meta FROM otp_codes WHERE channel = ? AND destination = ? AND purpose = ? AND consumed = 0 AND expires_at > ? ORDER BY id DESC LIMIT 1`,
        [channel, dest, pk, now],
        (err, row) => {
            if (err) return cb(err);
            cb(null, readOtpFromMeta(row && row.meta));
        }
    );
}
...
function saveOtp(db, { channel, destination, purpose, meta }, code, cb) {
    const dest = normalizeOtpDestination(channel, destination);
    const pk = otpRowPurpose(purpose, meta);
    const metaStr = metaForStore(purpose, meta, code);
...
function prepareOtpSend(db, { channel, destination, purpose, meta }, cb) {
    meta = meta || {};
    const finishPrepare = (rate) => {
        const pickCode = (next) => {
            if (rate.activeReuse && isPhoneAuthPurpose(purpose)) {
                return getActiveAuthOtpCode(db, { channel, destination, purpose, meta }, (readErr, existingCode) => {
                    if (readErr) return cb(readErr);
                    if (existingCode) return next(existingCode);
                    next(generateOtpDigits());
                });
            }
            if (isPhoneAuthPurpose(purpose)) return next(generateOtpDigits());
            next(generateOtpForDestination(channel, destination, purpose, meta));
        };
        pickCode((code) => {
    ...
    if (isPhoneAuthPurpose(purpose) && channel === 'phone') {
        return invalidateLegacyApplicantOtps(db, channel, destination, () => start());
    }
    start();
}
    ...
    db.run(
        ...
        function (ierr) {
            ...
            if (this && this.changes === 0) {
                return cb(new Error('Could not store verification token. Try again.'));
            }
...
module.exports = {
    ...
    ensureOtpWhatsAppDeliverySchema,
    claimOtpWhatsAppDelivery,
    releaseOtpWhatsAppDelivery,
    readOtpFromMeta,
    isPhoneAuthPurpose
};
