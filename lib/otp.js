...
function generateOtpForDestination(channel, destination, purpose, meta) {
    ...
    const pk = resolveOtpPurpose(purpose, meta, channel, destination);
    const secret =
...
/** One OTP per phone for applicant signup + sign-in (same code on both tabs). */
function resolveOtpPurpose(purpose, meta, channel, destination) {
    if ((purpose === 'signup' || purpose === 'login') && channel && destination) {
        const dest = normalizeOtpDestination(channel, destination);
        if (dest) return `applicant:${dest}`;
    }
    return otpRowPurpose(purpose, meta);
}

const otpWhatsAppSent = new Map();

function otpWhatsAppSentKey(channel, destination, purpose, meta) {
    return resolveOtpPurpose(purpose, meta || {}, channel, destination);
}
                ...
                fs.appendFileSync(
                    ...
                    JSON.stringify({
                        ...
                        data: {
                            channel,
                            purpose,
                            resolvedPurpose: resolveOtpPurpose(purpose, meta, channel, destination),
...
function verifyOtpStrict(db, { channel, destination, purpose, code, meta, userId, seminarId }, cb) {
    ...
    if (purpose === 'signup' || purpose === 'login') {
        const legacy = otpRowPurpose(purpose, meta);
        if (legacy !== pk) pks.push(legacy);
    }
