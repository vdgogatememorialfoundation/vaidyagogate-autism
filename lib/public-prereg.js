...
function seminarAllowsPublicPrereg(seminar, flow) {
    if (!seminar || !seminar.is_active) return { ok: false, error: 'Event not found or inactive.' };
    const flags = flow || seminarRegFlow.seminarFlowFlagsFromRegistrationFormJson(seminar.registration_form_json);
    if (!flags.preregistrationRequired) {
        return { ok: false, error: 'Pre-registration is not used for this event.' };
    }
    if (!flags.publicPreregEnabled) {
        return { ok: false, error: 'Public pre-registration link is not enabled for this event.' };
    }
    const win = seminarRegFlow.preregistrationWindowState(seminar, require('./seminar-datetime'));
    if (!win.open) {
        const msg =
    ...
    return { ok: true, flags, window: win };
}
