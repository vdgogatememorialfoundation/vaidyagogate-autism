...
function getPool() {
    if (!pool) {
        ...
        pool = new Pool({
            ...
            connectionTimeoutMillis: process.env.VERCEL ? 10000 : 25000,
...
async function syncSerialSequences() {
    const { rows } = await querySchemaWithRetry(
        `SELECT s.sequencename, c.relname AS table_name
           FROM pg_sequences s
           JOIN pg_class c ON c.relname = regexp_replace(s.sequencename, '_id_seq$', '')
           JOIN pg_namespace n ON n.oid = c.relnamespace AND n.nspname = s.schemaname
          WHERE s.schemaname = 'public' AND s.sequencename LIKE '%_id_seq'`,
        [],
        1
    );
    for (const r of rows || []) {
        try {
            await querySchemaWithRetry(
                `SELECT setval('${r.sequencename}', GREATEST(COALESCE((SELECT MAX(id) FROM "${r.table_name}"), 0), COALESCE((SELECT last_value FROM "${r.sequencename}"), 0)) + 1, false)`,
                [],
                1
            );
        } catch (e) {
            console.warn('[pg-schema] sequence sync failed for', r.sequencename, e.message);
        }
    }
}
        ...
        schemaReady = applyPostgresSchema()
            .then(async () => {
                ...
                try {
                    ...
                    console.warn('[pg-schema] sequence sync:', seqErr.message);
                }
                try {
                    await querySchemaWithRetry(
...
function runQuery(sql, params, callback, ctx) {
    ...
    queryWithRetry(pgSql, pgParams, 2)
        .then((result) => {
            const fake = {
                lastID: addReturning && result.rows && result.rows[0] ? result.rows[0].id : undefined,
...
async function runEnsureAuxiliaryTables() {
    if (runEnsureAuxiliaryTables._done) return [];
    if (runEnsureAuxiliaryTables._pending) return runEnsureAuxiliaryTables._pending;
    runEnsureAuxiliaryTables._pending = ensureAuxiliaryTables(querySchemaWithRetry, isIgnorablePgError)
        .then((missing) => {
            if (!missing || !missing.length) runEnsureAuxiliaryTables._done = true;
            return missing;
        })
        .finally(() => {
            runEnsureAuxiliaryTables._pending = null;
        });
    return runEnsureAuxiliaryTables._pending;
}
