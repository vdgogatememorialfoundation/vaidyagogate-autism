...
function runQuery(sql, params, callback, ctx) {
    ...
    // Debug logging for INSERT queries
    const isInsert = /^\s*INSERT\s+INTO/i.test(String(sql || ''));
    if (isInsert) {
        console.log('[pg-runQuery] INSERT:', {
            originalSql: sql.substring(0, 80),
            finalSql: pgSql.substring(0, 120),
            addReturning
        });
    }
    ...
    queryWithRetry(pgSql, pgParams, 2)
        .then((result) => {
            if (isInsert) {
                console.log('[pg-runQuery] INSERT result:', {
                    rowCount: result.rowCount,
                    rowsLength: result.rows ? result.rows.length : 0,
                    firstRow: result.rows && result.rows[0] ? JSON.stringify(result.rows[0]) : null,
                    addReturning
                });
            }
            const fake = {
            ...
            if (isInsert) {
                console.log('[pg-runQuery] Calculated lastID:', fake.lastID);
            }
            if (typeof callback === 'function') callback.call(fake, null);
        })
        .catch((err) => {
            if (isInsert) {
                console.error('[pg-runQuery] INSERT error:', err.message);
            }
