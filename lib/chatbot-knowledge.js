...
async function buildChatbotKnowledge(db, loadPublicSiteCms) {
    ...
    if (Array.isArray(cms.doctorUpdates)) {
        cms.doctorUpdates.forEach((u) => {
            if (u && u.title) parts.push(`Applicant portal update: ${u.title} — ${u.body || ''}`);
        });
    }
    ...
    tickets.forEach((t) => {
        parts.push(`Support ticket ${t.ticket_id_string || ''} (${t.status}) — category/support handled in applicant portal under Feedback & Support.`);
    });
    ...
    parts.push(
        'Support: use Feedback & Support in the applicant portal to raise a ticket; admins reply in the same thread (no live chat).'
    );
    parts.push(
...
function answerFromKnowledge(message, knowledge, userContext) {
    ...
    if (m.includes('support') || m.includes('ticket') || m.includes('help')) {
        reply =
            'Open the applicant portal → Feedback & Support → New support ticket. You can continue the conversation in the same thread when an admin replies.';
        return reply;
    }
    ...
    if (m.includes('gallery') || m.includes('past seminar')) {
        reply = 'Past seminar photos are shown in the Gallery section on the homepage when uploaded in Admin → Website & applicant updates.';
        return reply;
    }
    ...
    if (m.includes('seminar') || m.includes('register') || m.includes('fee') || m.includes('date')) {
        ...
        reply = hits.length ? hits.slice(0, 6).join('\n') : 'Browse seminars in the applicant portal under Available Seminars.';
        return reply;
    }
    ...
    if (m.includes('schedule') || m.includes('timing')) {
        ...
        reply = hits.length ? hits.slice(0, 8).join('\n') : 'See the Schedule section on the website or event details in your applicant portal.';
        return reply;
    }
    ...
    if (m.includes('case') || m.includes('agnikarma') || m.includes('viddhakarma') || m.includes('abstract')) {
        ...
        reply = hits.length ? hits.join('\n') : 'Case presentation applications are in the applicant portal under Case presentation application.';
        return reply;
    }
