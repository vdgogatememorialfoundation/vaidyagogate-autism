        ...
        `SELECT uc.*, s.title AS seminar_title, s.description AS seminar_description, s.event_date, s.location_url,
         ...
         WHERE uc.id = ? AND uc.user_id = ? AND uc.enabled = 1
           AND EXISTS (
               ...
               WHERE cs.user_id = uc.user_id 
                 AND cs.seminar_id = uc.seminar_id 
                 AND cs.category LIKE 'child_%'
           )`,
        [certId, userId],
        (err, row) => {
...
function buildRenderContext(row, certType, cmsVenue, logoUrl, configOverride) {
    ...
    try {
        ...
        if (fd.child_name && String(fd.child_name).trim()) childName = String(fd.child_name).trim();
    } catch(e) {}

    const recipientRaw = childName || displayNameWithHonorific(row.display_name, row.form_data, cfg.autoHonorific !== false);
    ...
    return {
        ...
        certificateId: escHtml('CERT-' + String(row && row.seminar_id || '0') + '-' + String(row && row.id || '0')),
        prnNo: vars.prn_no,
...
function renderImageCertificateHtml(ctx, templatePath) {
    const safePath = escHtml(String(templatePath || ''));
    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${ctx.certificateId} — Certificate</title><style>*{box-sizing:border-box}html,body{margin:0;background:#eef2ff;font-family:Inter,Arial,sans-serif;color:#24104f}.certificate-image-page{min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:18px}.certificate-canvas{position:relative;width:min(100%,1200px);aspect-ratio:3/2;background:#fff;box-shadow:0 18px 46px rgba(30,41,59,.22);overflow:hidden}.certificate-canvas>img{display:block;width:100%;height:100%;object-fit:cover}.certificate-overlay{position:absolute;inset:0;text-align:center;color:#21104d}.certificate-recipient{position:absolute;left:16%;right:16%;top:65.2%;font-family:Georgia,serif;font-size:clamp(18px,2.35vw,34px);font-weight:700;line-height:1.1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.certificate-date{position:absolute;left:17%;top:82.25%;font-family:Georgia,serif;font-size:clamp(10px,1.15vw,17px);font-weight:600}.certificate-id{position:absolute;left:43%;top:82.25%;font-family:Georgia,serif;font-size:clamp(10px,1.15vw,17px);font-weight:600}.cert-actions{display:flex;gap:10px;justify-content:center;margin:16px 0 6px}.cert-actions button{border:0;border-radius:999px;padding:11px 18px;color:#fff;font-weight:700;cursor:pointer;background:#4f46e5}.cert-actions button+button{background:#0f766e}.cert-print-hint{font-size:13px;color:#475569;text-align:center}.no-print{display:block}@media print{body{background:#fff}.certificate-image-page{padding:0}.certificate-canvas{width:100%;box-shadow:none}.no-print{display:none!important}}@media(max-width:640px){.certificate-image-page{padding:8px}.certificate-recipient{top:65%;font-size:18px}.certificate-date,.certificate-id{top:82%;font-size:10px}}</style></head><body><main class="certificate-image-page"><div class="certificate-canvas"><img src="${safePath}" alt="Certificate template"><div class="certificate-overlay"><div class="certificate-recipient">${ctx.recipientName}</div><div class="certificate-date">${ctx.eventDate}</div><div class="certificate-id">${ctx.certificateId}</div></div></div><div class="cert-actions no-print"><button type="button" onclick="downloadCertificateToDevice()">Download to device</button><button type="button" onclick="window.print()">Print / Save as PDF</button></div><p class="cert-print-hint no-print">Download an offline HTML copy or print this certificate as PDF.</p></main><script>function downloadCertificateToDevice(){var u=new URL(window.location.href);u.searchParams.set('download','1');window.location.href=u.toString();}</script></body></html>`;
}
...
<div class="cert-actions no-print"><button type="button" class="cert-download-btn" onclick="downloadCertificateToDevice()">Download to device</button><button type="button" class="cert-print-btn" onclick="window.print()">Print / Save as PDF</button></div>
<p class="cert-print-hint no-print">Use Download to device for an offline HTML copy, or Print / Save as PDF for a PDF copy.</p>
<script>
function downloadCertificateToDevice(){var u=new URL(window.location.href);u.searchParams.set('download','1');window.location.href=u.toString();}
document.addEventListener('contextmenu',function(e){e.preventDefault();});
window.addEventListener('load',function(){document.querySelector('.cert-page')?.classList.add('ready');});
</script>
</body>
</html>`;
}
...
function handleViewRequest(db, req, res) {
    ...
    const sendRenderError = (e) => {
        console.error('[certificate/view] render:', e && e.message ? e.message : e);
        if (res.headersSent) return;
        res.status(500).json({
            error: 'Certificate could not be rendered',
            detail: process.env.VERCEL_ENV === 'production' ? undefined : e && e.message ? e.message : String(e)
        });
    };
    ...
    const finish = (err, row, certType) => {
        if (err) {
            ...
            return res.status(500).json({
                ...
                detail: process.env.VERCEL_ENV === 'production' ? undefined : err.message
            });
        }
        ...
        loadCmsVenue(db, (eVenue, cmsVenue) => {
            if (eVenue) return sendRenderError(eVenue);
            loadLogoUrl(db, (eLogo, logoUrl) => {
                if (eLogo) return sendRenderError(eLogo);
                try {
                    const ctx = buildRenderContext(row, type, cmsVenue, logoUrl);
                    const filename = req.query && String(req.query.download || '') === '1' ? ctx.certificateId + '.html' : '';
                    const html = isBuiltinPath(path) ? renderCertificateHtml(ctx) : renderImageCertificateHtml(ctx, path);
                    sendHtml(html, filename);
                } catch (e) {
                    sendRenderError(e);
                }
            });
...
function applyBuiltinTemplate(db, { seminarId, certType, adminUserId }, cb) {
    ...
    const configJson = certCfg.stringifyConfig(certCfg.parseConfig(null));
