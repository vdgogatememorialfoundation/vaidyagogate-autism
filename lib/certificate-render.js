...
const BUILTIN_COMPETITION = '__builtin_vgmf_competition__';

const BUILTIN_BY_TYPE = {
    participant: BUILTIN_PARTICIPANT,
    volunteer: BUILTIN_VOLUNTEER,
    competition: BUILTIN_COMPETITION
};

function isBuiltinPath(filePath) {
    const p = String(filePath || '');
    return p === BUILTIN_PARTICIPANT || p === BUILTIN_VOLUNTEER || p === BUILTIN_COMPETITION;
}

function builtinCertType(filePath) {
    if (filePath === BUILTIN_VOLUNTEER) return 'volunteer';
    if (filePath === BUILTIN_COMPETITION) return 'competition';
    return 'participant';
}

function builtinPathForType(certType) {
    return BUILTIN_BY_TYPE[certKinds.normalizeCertKind(certType)];
}
...
function fetchCompetitionCert(db, certId, userId, cb) {
    db.get(
        `SELECT cc.id, cc.user_id, cc.seminar_id, cc.display_name, cc.enabled, cc.verify_token,
                s.title AS seminar_title, s.description AS seminar_description, s.event_date, s.location_url,
                ct.file_path AS template_path, ct.cert_type, ct.config_json, 'competition' AS kind,
                u.user_id_string, u.email, u.phone,
                cs.application_no, cs.title AS competition_title, cs.category AS competition_category,
                cs.form_data AS competition_form_data, cs.status AS competition_status
         FROM competition_certificates cc
         JOIN seminars s ON s.id = cc.seminar_id
         JOIN users u ON u.id = cc.user_id
         LEFT JOIN certificate_templates ct ON ct.id = cc.template_id
         LEFT JOIN competition_submissions cs ON cs.id = cc.submission_id
         WHERE cc.id = ? AND cc.user_id = ? AND cc.enabled = 1`,
        [certId, userId],
        (err, row) => {
            if (err) return cb(err);
            if (!row) return cb(null, null);
            if (!row.verify_token) {
                return certVerify.ensureCompetitionCertVerifyToken(db, certId, (e2, tok) => {
                    if (e2) return cb(e2);
                    row.verify_token = tok;
                    cb(null, row);
                });
...
function buildRenderContext(row, certType, cmsVenue, logoUrl, configOverride) {
    ...
    const kind = certKinds.normalizeCertKind(certType);
    const recipientRaw =
        kind === 'competition'
            ? certKinds.competitionRecipientName(
                  row.competition_form_data,
                  row.competition_category,
                  row.display_name
              )
            : displayNameWithHonorific(row.display_name, row.form_data, cfg.autoHonorific !== false);
    ...
    const vars = {
        ...
        competition_title: escHtml(String(row.competition_title || '').trim()),
        competition_category: escHtml(certKinds.competitionCategoryLabel(row.competition_category))
    };
    const bodyTpl =
        kind === 'volunteer'
            ? cfg.bodyVolunteer
    ...
    return {
        ...
        applicationNoLabel: kind === 'competition' ? 'Entry No.' : 'Application No.',
  ...
  <div class="cert-inner">
    ...
    <div class="cert-ids-row">
      <div class="cert-ids-text">
        ...
        <p class="cert-meta cert-id-line"><strong>${ctx.applicationNoLabel}</strong> ${ctx.applicationNo}</p>
      </div>
...
function sampleRowForPreview(seminarRow) {
    return {
        ...
        competition_title: 'My Colourful World',
        competition_category: 'child_drawing',
        competition_form_data: JSON.stringify({ child_name: 'Aarav Gogate' })
    };
}
...
function handleViewRequest(db, req, res) {
    ...
    const ccId = parseInt(req.query.cc, 10);
        ...
        loadCmsVenue(db, (eVenue, cmsVenue) => {
            ...
            loadLogoUrl(db, (eLogo, logoUrl) => {
                ...
                try {
                    ...
                    sendHtml(renderCertificateHtml(ctx));
                } catch (e) {
                    sendRenderError(e);
                }
            });
    ...
    if (Number.isInteger(ccId) && ccId > 0) {
        return fetchCompetitionCert(db, ccId, userId, (err, row) => finish(err, row, 'competition'));
    }
    ...
    return res.status(400).send('Specify uc, vc or cc certificate id.');
}
...
function applyBuiltinTemplate(db, { seminarId, certType, adminUserId }, cb) {
    const sid = parseInt(seminarId, 10);
    const type = certKinds.normalizeCertKind(certType);
    const filePath = builtinPathForType(type);
        ...
        () => {
            db.run(
                ...
                function (err) {
                    ...
                    db.run(
                        `UPDATE ${certKinds.certTable(type)} SET template_id = ?, updated_at = CURRENT_TIMESTAMP WHERE seminar_id = ? AND enabled = 1`,
                        [templateId, sid],
