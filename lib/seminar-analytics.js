...
function normalizeName(str, type) {
    if (!str) return '';
    const s = String(str).trim();
    if (!s) return '';
    if (s.length <= 4 && (type === 'qual' || type === 'state')) {
...
function inc(map, key, n, type) {
    let k = key && String(key).trim() ? String(key).trim() : '';
    if (k) {
        k = normalizeName(k, type);
    } else {
        k = '(blank)';
    }
    map[k] = (map[k] || 0) + (n || 1);
...
function topEntries(map, limit) {
    return Object.entries(map)
        .filter(([name]) => name && name !== '(blank)' && name.toLowerCase() !== 'undefined' && name.toLowerCase() !== 'null')
        .sort((a, b) => b[1] - a[1])
        .slice(0, limit || 25)
...
function computeSeminarAnalytics(rows, orders, preregs) {
    ...
    (rows || []).forEach((r) => {
        ...
        inc(byState, fd.state, 1, 'state');
        inc(byCity, fd.city, 1, 'city');
        inc(byPin, fd.pin, 1, 'pin');
        inc(byQual, fd.qual, 1, 'qual');
        inc(byCollege, fd.college, 1, 'college');
