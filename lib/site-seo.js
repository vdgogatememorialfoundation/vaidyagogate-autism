...
function normalizeFaviconUrl(raw) {
    const v = String(raw || DEFAULT_SEO.faviconUrl).trim() || DEFAULT_SEO.faviconUrl;
    if (v === '/favicon.svg' || v.startsWith('/api/branding/logo')) return '/favicon.ico';
    return v;
}
...
/** @deprecated use siteFavicon.serveFaviconPng */
function serveBrandingFavicon(db, res) {
    siteFavicon.serveFaviconPng(db, res, 32);
}
...
function registerSiteSeoRoutes(app, deps) {
    ...
    app.get('/apple-touch-icon.png', (req, res) => {
        siteFavicon.serveFaviconPng(db, res, 180);
    });
}
