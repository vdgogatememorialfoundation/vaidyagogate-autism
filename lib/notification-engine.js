...
let cachedOtpWaTemplateName = '';
let cachedOtpWaTemplateAt = 0;
const OTP_WA_TEMPLATE_TTL_MS = 300000;
...
async function sendOtpMessages({ email, phone, code, db, eventKey }) {
    ...
    const needEmailTpl = !!(email && isEmailConfigured());
    const needWhatsApp = !!(phone && isWhatsAppConfigured());
    const tpl = needEmailTpl && db ? await loadTemplateAsync(db, key, null).catch(() => null) : null;
    ...
    if (phone && isWhatsAppConfigured() && !results.sms.ok) {
        const waBody = tpl
            ...
            : 'Your verification code is ' + code + '. Valid for a short time.';
        const otpTplName = needWhatsApp && db ? await getOtpWhatsAppTemplateName(db) : '';
        if (otpTplName) {
...
async function getOtpWhatsAppTemplateName(db) {
    if (cachedOtpWaTemplateName && Date.now() - cachedOtpWaTemplateAt < OTP_WA_TEMPLATE_TTL_MS) {
        return cachedOtpWaTemplateName;
    }
    ...
    if (!fromIntegration && !fromDb && process.env.WHATSAPP_OTP_TEMPLATE_NAME) {
    ...
    if (name) {
        cachedOtpWaTemplateName = name;
        cachedOtpWaTemplateAt = Date.now();
    }
    return name;
}
