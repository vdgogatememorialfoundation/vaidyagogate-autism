...
const OTP_TEMPLATE_CACHE_MS = 5 * 60 * 1000;
let otpWaTemplateCache = { name: null, at: 0 };

function invalidateOtpTemplateCache() {
    otpWaTemplateCache = { name: null, at: 0 };
}
...
async function sendOtpMessages({ email, phone, code, db, eventKey, vars: extraVars }) {
    const portalTitle = require('./portal-product').FEATURES.portalTitle || 'Autism Awareness Portal';
    const vars = Object.assign(
        {
            ...
            seminar_name: portalTitle,
            portal_name: portalTitle,
            login_url: publicBaseUrl() || '',
            login_email: email || '',
            ttl_minutes: String(require('./otp').OTP_TTL_MIN)
        },
    ...
    const needsTemplate =
        ...
        (phone && isMsg91Configured());
    const tpl = needsTemplate && db ? await loadTemplateAsync(db, key, null).catch(() => null) : null;
    ...
    if (phone && isMsg91Configured() && !results.whatsapp.ok) {
        const smsText = tpl
...
async function getOtpWhatsAppTemplateName(db) {
    ...
    if (otpWaTemplateCache.name && Date.now() - otpWaTemplateCache.at < OTP_TEMPLATE_CACHE_MS) {
        return otpWaTemplateCache.name;
    }
    let fromIntegration = '';
    let fromDb = '';
    let fromEnv = '';
    ...
    const name = sanitizeWhatsAppTemplateName(fromIntegration || fromDb || fromEnv);
    otpWaTemplateCache = { name, at: Date.now() };
    return name;
}
