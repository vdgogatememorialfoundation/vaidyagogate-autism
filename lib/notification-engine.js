...
function buildForgotPasswordLink(returnTo, token) {
    ...
    let raw = String(returnTo || 'index.html').trim();
...
function buildVarsFromRows(user, seminar, reg, order, extra) {
    ...
    const vars = {
        ...
        seminar_name: (seminar && seminar.title) || require('./notification-defaults').SEMINAR,
        seminar_date:
...
async function sendForgotPasswordEmail(db, userId, forgotPasswordLink, cb) {
    try {
        ...
        if (!picked) {
            subject = 'Password reset – ' + require('./notification-defaults').SEMINAR;
            html =
