...
async function resolveWabaId() {
    ...
    if (phoneNumberId) {
        try {
            ...
            if (waba && typeof waba === 'object' && waba.id) {
                cachedWabaId = String(waba.id);
                cachedWabaAt = Date.now();
                return cachedWabaId;
            }
    ...
    try {
        ...
        for (const biz of bizRes.data?.data || []) {
            try {
                ...
                if (wabas.length === 1) {
                    cachedWabaId = String(wabas[0].id);
                    cachedWabaAt = Date.now();
                    return cachedWabaId;
                }
                if (wabas.length > 1) {
...
const debugLookupCache = new Map();
const DEBUG_LOOKUP_TTL_MS = 1800000;
const otpSendRouteCache = new Map();
const OTP_ROUTE_TTL_MS = 86400000;
...
async function debugWhatsAppTemplateLookupUncached(templateName) {
    const tplName = sanitizeWhatsAppTemplateName(templateName);
...
async function debugWhatsAppTemplateLookup(templateName) {
    const tplName = sanitizeWhatsAppTemplateName(templateName);
    if (!tplName) return debugWhatsAppTemplateLookupUncached(templateName);
    const cached = debugLookupCache.get(tplName);
    if (cached && Date.now() - cached.at < DEBUG_LOOKUP_TTL_MS) {
        return cached.data;
    }
    const data = await debugWhatsAppTemplateLookupUncached(tplName);
    debugLookupCache.set(tplName, { at: Date.now(), data });
    return data;
}
...
async function sendOtpUsingCachedRoute(to, tplName, otp, route) {
    const templatePayload = {
        name: tplName,
        language: route.langVariant || { code: route.langCode || 'en' }
    };
    if (route.components && route.components.length) {
        templatePayload.components = route.components;
    }
...
async function sendWhatsAppOtpTemplate(phone, templateName, code) {
    ...
    const cachedRoute = otpSendRouteCache.get(tplName);
    if (cachedRoute && Date.now() - cachedRoute.at < OTP_ROUTE_TTL_MS) {
        const fast = await sendOtpUsingCachedRoute(to, tplName, otp, cachedRoute);
        if (fast.ok) {
            return Object.assign(
                {
                    method: cachedRoute.method,
                    lang: cachedRoute.langCode,
                    template: tplName,
                    fastPath: true
                },
                fast
        ...
        otpSendRouteCache.delete(tplName);
    }
    ...
    for (const langCode of langCandidates) {
        ...
        for (const langVariant of buildTemplateLanguageVariants(langCode)) {
            for (const attempt of componentAttempts) {
                ...
                if (r.ok) {
                    otpSendRouteCache.set(tplName, {
                        at: Date.now(),
                        langCode,
                        langVariant,
                        method: attempt.label,
                        components: attempt.components || null
                    });
                    return Object.assign(
                        {
                            method: attempt.label,
