...
/** Template variables for the LOGIN_OTP email/WhatsApp (who, which portal, where to sign in). */
...
function deliverLoginOtp(channel, dest, code, db, row, portal) {
    const vars = loginOtpVars(row, portal);
    if (channel === 'phone') {
        return notifEngine
            .sendOtpMessages({ phone: dest, code, db, eventKey: 'LOGIN_OTP', vars })
            .then((r) => {
    ...
    channels.forEach(({ channel, dest }) => {
        otpLib.prepareOtpSend(db, { channel, destination: dest, purpose: 'login', meta }, (serr, code, id, codeReused) => {
            ...
            const afterClaim = (shouldSend) => {
                ...
                deliverLoginOtp(channel, dest, code, db, row, 'public').then((sent) => {
...
function sendLoginOtpChannel(db, row, channel, cb, opts) {
    ...
    otpLib.prepareOtpSend(db, { channel, destination: dest, purpose: 'login', meta }, (serr, code, id, codeReused) => {
        ...
        const sendWa = () => {
            deliverLoginOtp(channel, dest, code, db, row, portal).then((sent) => {
                if (channel === 'phone' && !sent.ok && !sent.skipped) {
                    otpLib.releaseOtpWhatsAppDelivery(db, id, () => {});
                }
                if (channel === 'phone' && sent.ok) {
