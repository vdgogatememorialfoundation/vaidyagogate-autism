...
/** Allow staff UI assets when the request comes from an open /admin tab during maintenance. */
function isAdminRefererBypass(req) {
    const ref = String(req.headers.referer || req.headers.referrer || '');
    if (!ref) return false;
    try {
        const path = new URL(ref).pathname || '';
        return path === '/admin' || path.startsWith('/admin/') || path.endsWith('/admin.html');
    } catch (_) {
        return /\/admin(?:\.html)?(?:\?|#|$)/i.test(ref);
    }
}

function isStaffAssetPath(pathname) {
    const p = pathname || '/';
    return (
        p.startsWith('/api/qrcode/') ||
...
function createSiteKillSwitchMiddleware(db) {
    return function siteKillSwitchMiddleware(req, res, next) {
        ...
        maintenanceSettings.readMaintenanceBundle(db, (err, bundle) => {
            ...
            if (isAdminRefererBypass(req) && isStaffAssetPath(req.path)) return next();
