        ...
        requireAdminActor(req, res, () => {
            ...
            db.all(
                ...
                (err, rows) => {
                    ...
                    const pax = (a) => (Number(a.attendees_count) >= 1 ? Number(a.attendees_count) : 1);
                    const peopleExpected = attendees.reduce((s, a) => s + pax(a), 0);
                    const peopleArrived = attendees.filter((a) => a.checked_in).reduce((s, a) => s + pax(a), 0);
                    res.json({
                        attendees,
                        ...
                        peopleExpected,
                        peopleArrived,
                        peopleComing: Math.max(0, peopleExpected - peopleArrived)
                    });
                }
            );
        });
