...
function isNoFeesPortal() {
    return !!(portalProduct.FEATURES && portalProduct.FEATURES.noFees);
}

function portalFoundationName(row) {
    if (row && row.foundation_name) return String(row.foundation_name);
    if (isNoFeesPortal() && portalProduct.FEATURES.foundationName) {
        return portalProduct.FEATURES.foundationName;
    }
    return branding.FOUNDATION_NAME;
}

function portalSubtitle(row) {
    const year = resolvePortalYear(row);
    if (isNoFeesPortal()) {
        const label = portalProduct.FEATURES.eventLabel || 'Autism Awareness Programme';
        return year ? `${label} ${year}` : label;
    }
    return year ? `National Seminar ${year}` : 'National Seminar';
}

function entryGranted(row) {
    if (isNoFeesPortal()) return row.is_valid !== 0 && row.is_valid !== false;
...
function buildQrBlock(row, qrDataUrl) {
    ...
    const noFees = isNoFeesPortal();
    const paid = entryGranted(row);
    ...
    let statusBelow = '';
    if (paid && !invalid && !scanned) {
        if (noFees) {
            statusBelow = '<p class="qr-paid-below" aria-hidden="true">FREE</p>';
            const count =
                row.attendees_count != null
                    ? ticketAttendees.parseAttendeesCount(row.attendees_count, null)
                    : ticketAttendees.attendeesCountFromFormData(row.form_data);
            if (count != null) {
                const passLabel = ticketAttendees.attendeesValidityLabel(count);
                if (passLabel) {
                    statusBelow +=
                        '<p class="qr-entry-pass" aria-hidden="true">' + escapeHtml(passLabel) + '</p>';
                }
            }
        } else {
            statusBelow = '<p class="qr-paid-below" aria-hidden="true">PAID</p>';
        }
    }

    return `<div class="qr-wrap">${qrImg}${stampHtml}${statusBelow}${hint}</div>`;
}
...
function buildTicketHtml(row, qrDataUrl) {
    ...
    const foundation = escapeHtml(portalFoundationName(row));
    const portalLine = escapeHtml(portalSubtitle(row));
    ...
    const entryPassLine = ticketAttendees.attendeesValidityHtml(row, { escapeHtml });
...
.qr-entry-pass{margin:6px 0 0;font-size:.78rem;font-weight:700;color:#0f766e;text-align:center;line-height:1.35;max-width:168px;}
...
<div class="card">
  <div class="head">
    <div class="brand-row">
      ...
      <div class="brand-text">
        ...
        <div class="portal-year">${portalLine}</div>
      </div>
    </div>
    <h1>E-Ticket</h1>
  ...
  <div class="foot">${foundation} · ${portalLine} · Computer-generated e-ticket</div>
</div>
<script>
...
async function buildTicketHtmlFromRow(row, db) {
    ...
    if (!r.foundation_name) r.foundation_name = portalFoundationName(r);
    let qrDataUrl = '';
    if (r.qr_code_data) {
        try {
