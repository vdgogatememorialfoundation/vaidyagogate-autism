...
function buildQrBlock(row, qrDataUrl) {
    ...
    if (paid && !invalid && !scanned) {
        statusBelow = noFees
            ? '<p class="qr-paid-below" aria-hidden="true">FREE</p>'
            : '<p class="qr-paid-below" aria-hidden="true">PAID</p>';
        const count = ticketAttendees.resolveAttendeesCount(row);
        if (count != null) {
            const passLabel = ticketAttendees.attendeesValidityLabel(count);
            if (passLabel) {
                statusBelow +=
                    '<p class="qr-entry-pass" aria-hidden="true">' +
                    escapeHtml(count + ' attendee' + (count === 1 ? '' : 's') + ' · ' + passLabel) +
                    '</p>';
            }
        }
    }
