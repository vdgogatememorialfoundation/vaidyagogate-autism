...
/** Staff portals that sign in with an email OTP instead of a password. */
const STAFF_EMAIL_OTP_PORTALS = new Set(['admin', 'staff']);
...
function merge(raw) {
    ...
    // Applicant sign-in is always passwordless email OTP.
    const requireLoginOtp = true;
    const passwordlessLogin = true;
    debugAuthPolicyLog(
...
/** Admin console and staff portal sign in with email OTP (no password). */
function staffEmailOtpLogin(portal) {
    return STAFF_EMAIL_OTP_PORTALS.has(normalizeLoginPortal(portal));
}
...
function loginOtpChannels() {
    ...
    return { whatsapp, email: true };
}

function applicantLoginOtpRequired(portal) {
    if (isStaffPortal(portal)) return staffEmailOtpLogin(portal);
