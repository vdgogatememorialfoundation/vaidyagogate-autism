...
/** Staff-side login portals (not the public applicant portal). */
const STAFF_LOGIN_PORTALS = new Set(['admin', 'staff', 'judge', 'scanner']);
/** Portals that sign in with an email OTP instead of a password. */
const EMAIL_OTP_LOGIN_PORTALS = new Set(['staff', 'judge', 'scanner']);
/** Super admin / admin console stays password-only. */
const PASSWORD_ONLY_PORTALS = new Set(['admin']);
...
function merge(raw) {
    ...
    /* Enabling the applicant login-OTP channel means "OTP is the credential" (email OTP, no password).
       Legacy loginOtpWhatsapp=true is honoured as "login OTP on" but delivery is always email. */
    const loginChannelOn = o.loginOtpEmail === true || o.loginOtpWhatsapp === true;
    if (o.passwordlessLogin === true || loginChannelOn) {
        requireLoginOtp = true;
    }
...
/** Sign-in OTP is email-only on every portal; WhatsApp is never used for sign-in. */
...
function isEmailOtpLoginPortal(portal) {
    return EMAIL_OTP_LOGIN_PORTALS.has(normalizeLoginPortal(portal));
}

function isPasswordOnlyPortal(portal) {
    return PASSWORD_ONLY_PORTALS.has(normalizeLoginPortal(portal));
}
...
/** Whether a password is skipped for this portal (OTP is the credential). */
function passwordlessForPortal(portal) {
    if (isEmailOtpLoginPortal(portal)) return true;
    if (isPasswordOnlyPortal(portal)) return false;
    return passwordlessLoginEnabled();
}

/** OTP channels for a portal: staff/scanner portals are always email-only. */
function loginOtpChannelsForPortal(portal) {
    if (isEmailOtpLoginPortal(portal)) return { whatsapp: false, email: true };
    if (isPasswordOnlyPortal(portal)) return { whatsapp: false, email: false };
    return loginOtpChannels();
}
...
function publicPortalAuthPayload() {
    ...
    return {
        ...
        staffLoginOtp: true,
        emailOtpLoginPortals: Array.from(EMAIL_OTP_LOGIN_PORTALS),
...
module.exports = {
    ...
    isEmailOtpLoginPortal,
    isPasswordOnlyPortal,
    passwordlessForPortal,
    loginOtpChannelsForPortal,
    EMAIL_OTP_LOGIN_PORTALS,
