...
function parseAttendeesCount(formData, fallback) {
    ...
    let raw = fd.attendees_count != null ? fd.attendees_count : fd.attendeesCount;
    if (raw == null) {
        const k = Object.keys(fd).find((key) => /attendee|no_of_person|num_person|persons|people|pax|guests/i.test(key));
        if (k) raw = fd[k];
    }
