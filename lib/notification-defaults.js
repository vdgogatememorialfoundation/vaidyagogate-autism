...
const DEFAULT_TEMPLATES = [
    ...
    },
    {
        event_key: 'LOGIN_OTP',
        channel: 'both',
        email_subject: '{{otp_code}} is your {{portal_name}} sign-in code — {{portal_title}}',
        email_html: wrapHtml(
            '{{portal_name}} sign-in code',
            '<p>Dear {{first_name}},</p>' +
                '<p>Use the one-time code below to sign in to the <strong>{{portal_name}}</strong> of the {{portal_title}}.</p>' +
                '<div style="margin:18px 0;padding:18px 20px;border-radius:12px;background:#f0fdf4;border:1px solid #bbf7d0;text-align:center;">' +
                '<p style="margin:0 0 6px;font-size:12px;letter-spacing:2px;text-transform:uppercase;color:#166534;">Your sign-in code</p>' +
                '<p style="margin:0;font-size:32px;font-weight:bold;letter-spacing:8px;color:#14532d;">{{otp_code}}</p>' +
                '</div>' +
                '<p>This code is valid for <strong>{{ttl_minutes}} minutes</strong> and can be used only once. ' +
                'Enter it on the {{portal_name}} sign-in page for <strong>{{login_email}}</strong>.</p>' +
                emailCtaButton('{{login_url}}', 'Open {{portal_name}}') +
                '<p style="font-size:13px;color:#b91c1c;"><strong>Never share this code.</strong> Our team will never ask you for it by phone, WhatsApp or email.</p>' +
                '<p style="font-size:13px;color:#64748b;">If you did not try to sign in, you can ignore this email — your account stays secure without any action.</p>'
        ),
        whatsapp_body:
            '🔐 {{portal_name}} sign-in\nHello {{first_name}},\nYour {{portal_title}} sign-in code is:\n{{otp_code}}\nValid for {{ttl_minutes}} minutes. Never share this code.'
    },
    {
    ...
    },
    {
        ...
        email_html: wrapHtml(
            ...
            '<p>Dear {{full_name}},</p><p>Your pre-registration for <strong>{{seminar_name}}</strong> has been received.</p>' +
                ...
                '<p>You can check status anytime without signing in — use the button below with the same email you submitted.</p>' +
                emailCtaButton('{{public_prereg_track_url}}', 'Track status live (no sign-in)')
        ),
        whatsapp_body:
            '✅ Pre-registration submitted\nHello {{first_name}},\nYour pre-registration for {{seminar_name}} was received.\n📄 ID: {{application_no}}\n📌 Status: Pending review\n🔍 Track live: {{public_prereg_track_url}}'
    },
    {
        ...
        email_html: wrapHtml(
            ...
            '<p>Dear {{full_name}},</p><p>Your pre-registration for <strong>{{seminar_name}}</strong> has been approved.</p>' +
                ...
                '<p>If final registration is already open, sign in below to complete it. Otherwise we will notify you when final registration opens.</p>' +
                emailCtaButton('{{portal_login_url}}', 'Sign in to applicant portal')
        ),
        whatsapp_body:
    ...
    },
    {
        ...
        channel: 'email',
        enabled: 1,
        email_subject: 'Applicant replied on support ticket',
        email_html: wrapHtml(
            'Applicant support reply',
