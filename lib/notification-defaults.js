...
const SEMINAR = 'Autism Awareness Programme';
const TEAM = 'Team Vaidya Gogate Memorial Foundation<br>Autism Awareness Programme Support';
...
const DEFAULT_TEMPLATES = [
    ...
    },
    {
        ...
        whatsapp_body:
            '💳 Payment Successful\nHello {{first_name}},\nYour payment for Autism Awareness Programme has been received successfully.\n💰 Amount: ₹{{payment_amount}}\n🧾 Payment ID: PAY-{{payment_id}}\n📌 Status: Paid{{whatsapp_group_line}}'
    },
    {
        event_key: 'TICKET_ISSUED',
        channel: 'both',
        email_subject: 'Your E-Ticket is Ready – Autism Awareness Programme',
        email_html: wrapHtml(
            'E-ticket ready',
    ...
    },
    {
        ...
        whatsapp_body:
            '🎉 Application Approved!\nHello {{first_name}},\nYour application for Autism Awareness Programme has been approved successfully.\n📄 Application ID: {{application_no}}\n📌 Status: Approved'
    },
    {
        event_key: 'APPLICATION_REJECTED',
        channel: 'both',
        email_subject: 'Application Status Update – Autism Awareness Programme',
        email_html: wrapHtml(
        ...
        whatsapp_body:
            'Hello {{first_name}},\nYour application for Autism Awareness Programme was reviewed.\n📄 Application ID: {{application_no}}\n📌 Status: Rejected\nFor assistance, contact support.'
    },
    {
        ...
        email_html: wrapHtml(
            ...
            '<p>Dear {{full_name}},</p><p>Your seminar application <strong>{{application_no}}</strong> was reviewed.</p>' +
                ...
                '<p>Sign in to the portal, open <strong>Track seminar applications</strong>, and re-upload on the <em>same application number</em>. No new application is needed.</p>' +
                emailCtaButton('{{portal_login_url}}', 'Open portal')
        ),
        whatsapp_body:
    ...
    },
    {
        ...
        channel: 'both',
        email_subject: 'Complete Your Payment – Autism Awareness Programme',
        email_html: wrapHtml(
            'Payment pending',
    ...
    },
    {
        ...
        channel: 'both',
        email_subject: 'Check-In Confirmed – Autism Awareness Programme',
        email_html: wrapHtml(
            'Check-in confirmed',
    ...
    },
    {
        ...
        channel: 'both',
        email_subject: 'Your E-Certificate is Ready – Autism Awareness Programme',
        email_html: wrapHtml(
            'Certificate ready',
    ...
    },
    {
        ...
        email_html: wrapHtml(
            ...
            '<p>Dear {{full_name}},</p><p>We received your support ticket <strong>{{ticket_id}}</strong> regarding <strong>{{ticket_subject}}</strong>.</p>' +
                ...
                '<p>Our team will reply in the portal. You can follow the conversation here:</p>' +
                emailCtaButton('{{portal_login_url}}', 'Open support tickets')
        ),
        whatsapp_body:
            'Hello {{first_name}}, we received your support ticket {{ticket_id}} ({{ticket_subject}}). Track replies in the portal: {{portal_login_url}}'
    },
    {
    ...
    },
    {
        ...
        email_html: wrapHtml(
            ...
            '<p>Dear {{full_name}},</p><p>Your payment for ' +
                SEMINAR +
                ' could not be completed. Please try again from <a href="{{portal_login_url}}">the portal</a>.</p>'
        ),
    ...
    },
    {
        ...
        channel: 'both',
        enabled: 1,
        email_subject: 'Check-in could not be completed – ' + SEMINAR,
        email_html: wrapHtml(
            'Check-in issue',
            '<p>Dear {{full_name}},</p><p>We could not complete check-in for your registration.</p>' +
                '<p><strong>Reason:</strong> {{check_in_failure_reason}}</p>' +
                '<p>Please contact support or visit the help desk if you need assistance.</p>' +
                emailCtaButton('{{portal_login_url}}', 'Open portal')
        ),
        whatsapp_body: 'Hello {{first_name}}, check-in could not be completed: {{check_in_failure_reason}}'
    },
    {
        event_key: 'REFUND_COMPLETED',
        channel: 'both',
        enabled: 1,
        email_subject: 'Refund processed – ' + SEMINAR,
        email_html: wrapHtml(
            'Refund completed',
            '<p>Dear {{full_name}},</p><p>Your refund of <strong>₹{{refund_amount}}</strong> has been processed.</p>' +
                '<p>It may take a few business days to appear in your account depending on your bank.</p>'
        ),
        whatsapp_body: 'Hello {{first_name}}, your refund of ₹{{refund_amount}} has been processed.'
    },
    {
        event_key: 'QR_TICKET_REISSUED',
        channel: 'both',
        enabled: 1,
        email_subject: 'Your e-ticket was re-sent – ' + SEMINAR,
        email_html: wrapHtml(
            'E-ticket re-sent',
            '<p>Dear {{full_name}},</p><p>Your e-ticket has been re-sent.</p>' +
                '<p><strong>Ticket ID:</strong> ET-{{ticket_id}}<br><strong>Participant ID:</strong> {{user_id_string}}</p>' +
                emailCtaButton('{{ticket_pdf_url}}', 'Open printable ticket (PDF)') +
                emailCtaButton('{{portal_login_url}}', 'View in portal')
        ),
        whatsapp_body: 'Hello {{first_name}}, your e-ticket ET-{{ticket_id}} was re-sent. Portal: {{portal_login_url}}'
    },
    {
        event_key: 'CERTIFICATE_REISSUED',
        channel: 'both',
        enabled: 1,
        email_subject: 'Your certificate was re-sent – ' + SEMINAR,
        email_html: wrapHtml(
            'Certificate re-sent',
            '<p>Dear {{full_name}},</p><p>Your participation certificate has been re-sent.</p>' +
                emailCtaButton('{{certificate_url}}', 'Download certificate')
        ),
        whatsapp_body: 'Hello {{first_name}}, your certificate is ready again: {{certificate_url}}'
    },
    {
        event_key: 'EVENT_STARTING_TODAY',
        channel: 'both',
        enabled: 1,
        email_subject: 'Today is event day – ' + SEMINAR,
        email_html: wrapHtml(
            'Event today',
    ...
    },
    {
        ...
        channel: 'both',
        enabled: 1,
        email_subject: 'Important update – ' + SEMINAR,
        email_html: wrapHtml(
            'Announcement',
            '<p>Dear {{full_name}},</p><p><strong>{{ticket_subject}}</strong></p><p>{{announcement_body}}</p>' +
                emailCtaButton('{{portal_login_url}}', 'Open portal')
        ),
        whatsapp_body: 'Hello {{first_name}}, update: {{ticket_subject}} — {{announcement_body}}'
    },
    {
        event_key: 'INVOICE_GENERATED',
        channel: 'both',
        enabled: 1,
        email_subject: 'Payment receipt – ' + SEMINAR,
        email_html: wrapHtml(
            'Payment receipt',
            '<p>Dear {{full_name}},</p><p>Your payment receipt is available.</p>' +
                '<p>Amount: ₹{{payment_amount}}<br>Payment ID: PAY-{{payment_id}}</p>' +
                emailCtaButton('{{invoice_url}}', 'View receipt')
        ),
        whatsapp_body: 'Hello {{first_name}}, payment receipt for ₹{{payment_amount}}: {{invoice_url}}'
    },
    {
        event_key: 'WAITLIST_CONFIRMED',
        channel: 'both',
        enabled: 1,
        email_subject: 'Waitlist confirmed – ' + SEMINAR,
        email_html: wrapHtml(
            'Waitlist confirmed',
