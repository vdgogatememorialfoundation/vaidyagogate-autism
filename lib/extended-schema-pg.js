...
const AUX_TABLE_DDL = [
    ...
    {
        name: 'competition_certificates',
        sql: `CREATE TABLE IF NOT EXISTS competition_certificates (
            id SERIAL PRIMARY KEY,
            user_id INTEGER NOT NULL,
            seminar_id INTEGER NOT NULL,
            submission_id INTEGER,
    ...
    {
        name: 'otp_whatsapp_deliveries',
        sql: `CREATE TABLE IF NOT EXISTS otp_whatsapp_deliveries (
    ...
    {
        name: 'onspot_links',
        sql: `CREATE TABLE IF NOT EXISTS onspot_links (
            id SERIAL PRIMARY KEY,
    ...
    {
        name: 'onspot_submissions',
        sql: `CREATE TABLE IF NOT EXISTS onspot_submissions (
            id SERIAL PRIMARY KEY,
            link_id INTEGER,
            seminar_id INTEGER NOT NULL,
            first_name TEXT,
...
async function ensureSupportTicketsColumns(queryWithRetry, isIgnorablePgError) {
    ...
    // Ensure support_messages table exists (used by doctor portal)
    try {
        await queryWithRetry(
        ...
        if (!isIgnorablePgError(e)) {
            console.warn('[pg-schema] support_messages table:', e.message);
        }
    }
    try {
        await queryWithRetry(
        ...
        if (!isIgnorablePgError(e)) {
            console.warn('[pg-schema] support_messages index:', e.message);
        }
    }
}
