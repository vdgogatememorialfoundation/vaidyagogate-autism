...
function repairSupportTicketConstraints(db, ignoreErr, cb) {
    if (!isPostgresEnv()) return cb && cb();
    db.run(
    ...
    function createMessagesTable() {
        ...
        db.run(msgSql, (e) => {
            ...
            db.run(
                ...
                (e2) => {
                    ignoreErr(e2);
                    repairSupportTicketConstraints(db, ignoreErr, next);
                }
            );
            });
        });
...
let supportTicketSchemaReady = false;

/** Run full support-ticket DDL once per process (not on every API request). */
function ensureSupportTicketSchemaOnce(db, ignoreErr, next) {
    if (supportTicketSchemaReady) {
        if (next) next();
        return;
    }
    ensureSupportTicketSchema(db, ignoreErr, () => {
        supportTicketSchemaReady = true;
        if (next) next();
    });
}

function resetSupportTicketSchemaCacheForTests() {
    supportTicketSchemaReady = false;
}

module.exports = {
    ensureSupportTicketSchema,
    ensureSupportTicketSchemaOnce,
    resetSupportTicketSchemaCacheForTests
};
