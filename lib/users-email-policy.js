...
const LOGIN_USER_COLUMNS = `id, user_id_string, first_name, middle_name, last_name, email, phone, password, role, user_role,
                is_disabled, COALESCE(is_banned, 0) AS is_banned, COALESCE(is_demo, 0) AS is_demo, admin_modules,
                doctor_category, doctor_modules,
                COALESCE(email_verified, 1) AS email_verified,
                created_at, activated_at, last_login_at`;

function pickLoginMatchForPortal(matches, portal) {
    if (!matches.length) return { row: null };
    ...
    if (portalNorm === 'admin') {
        ...
        if (admins.length === 1) return { row: admins[0] };
        if (admins.length > 1) {
            return {
                row: null,
                extra: {
    ...
    if (portalNorm && portalNorm !== 'public') {
        ...
        if (staff.length === 1) return { row: staff[0] };
    }
    const doctors = matches.filter((r) => userRoles.isDoctorPortalAccount(r));
    if (doctors.length === 1) return { row: doctors[0] };
    return {
        row: null,
        extra: {
            ambiguous: true,
...
/** Passwordless (OTP) login lookup: email or 12-digit portal ID, disambiguated by portal. */
function findUserForOtpLogin(db, { identifier, portal }, cb) {
    const id = String(identifier || '').trim();
    if (!id) return cb(null, null);
    if (isPortalIdLogin(id)) {
        const pid = normalizePortalId(id);
        if (!pid) return cb(null, null);
        return db.get(
            `SELECT ${LOGIN_USER_COLUMNS} FROM users
             WHERE REPLACE(TRIM(user_id_string), ' ', '') = ? AND ${authUsers.sqlUserActive('is_disabled')}`,
            [pid],
            cb
        );
    }
    ...
    db.all(
        ...
        (err, rows) => {
            if (err) return cb(err);
            const picked = pickLoginMatchForPortal(rows || [], portal);
            cb(null, picked.row, picked.extra || null);
        }
    );
}
...
function findUserForLogin(db, { identifier, password, portal, skipPassword }, cb) {
    const id = String(identifier || '').trim();
    if (!id) return cb(null, null);
    if (skipPassword) return findUserForOtpLogin(db, { identifier: id, portal }, cb);
    if (password === undefined || password === null) return cb(null, null);
