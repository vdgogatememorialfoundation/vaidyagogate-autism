/**
 * Admin single & bulk email compose (ZeptoMail via email-service / notification queue).
 */
...
function resolveRecipients(db, opts, cb) {
    ...
    // All registered users with email (doctors, judges, candidates - not staff/admin)
    if (audience === 'all_registered_users') {
        return db.all(
            `SELECT id, email, first_name, last_name FROM users
             WHERE IFNULL(is_disabled,0) = 0
               AND email IS NOT NULL AND trim(email) != ''
               AND (
                   lower(trim(IFNULL(user_role,''))) IN ('doctor', 'judge_user', 'candidate', 'volunteer', '')
                   OR role IN ('doctor', 'judge', 'volunteer', '')
               )
               AND (
                   lower(trim(IFNULL(user_role,''))) NOT IN ('superadmin', 'admin', 'staff', 'scanner_portal_user')
                   AND role NOT IN ('admin', 'superadmin', 'staff')
               )`,
            [],
            (e, rows) => {
                if (e) return cb(e);
    ...
    // Users who registered via public pre-registration form
    if (audience === 'public_form_registrants') {
        return db.all(
            `SELECT DISTINCT u.id, u.email, u.first_name, u.last_name
             FROM users u
             WHERE u.email IS NOT NULL AND trim(u.email) != ''
               AND u.user_id_string IS NOT NULL AND trim(u.user_id_string) != ''
               AND (
                   SELECT 1 FROM preregistrations p
                   WHERE p.user_id = u.id AND p.form_data LIKE '%public_prereg%'
                   LIMIT 1
               ) = 1
    ...
    // Users who have made any seminar registration (paid or unpaid)
    if (audience === 'registered_candidates') {
        return db.all(
            `SELECT DISTINCT u.id, u.email, u.first_name, u.last_name
             FROM users u
             WHERE EXISTS (
                 SELECT 1 FROM registrations r WHERE r.user_id = u.id AND r.status NOT IN ('rejected', 'cancelled')
             )
             AND IFNULL(u.is_disabled,0) = 0
...
async function sendSingleMail({ to, name, subject, body, replyTo }) {
    ...
    if (!isEmailConfigured()) return { ok: false, error: 'Email (ZeptoMail) is not configured in integrations' };
...
function sendBulkMail(db, { recipients, subject, body, useQueue = true, eventKey = 'ADMIN_COMPOSE' }, cb) {
    ...
    if (!isEmailConfigured()) return cb(new Error('Email (ZeptoMail) is not configured in integrations'));
    const sub = String(subject || '').trim();
    const b = String(body || '').trim();
