...
/**
 * Date-only columns (DATE / midnight UTC) must never render as "5:30 am":
 * a UTC-midnight timestamp is 05:30 IST, so treat it as a calendar day.
 */
function isDateOnlyValue(val, istHour, istMinute) {
    const raw = val instanceof Date ? val.toISOString() : String(val || '').trim();
...
/** Calendar day only (no time) in IST, e.g. "Saturday, 5 September 2026". */
function formatSeminarDateOnly(val) {
...
function formatSeminarDateForTicket(val) {
    ...
    if (isDateOnlyValue(val, hh, mm)) return { date: dateLine, time: null };
    const h12 = hh % 12 || 12;
