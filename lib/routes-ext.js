...
module.exports = function registerExtendedRoutes(app, deps) {
    ...
    app.get('/api/admin/certificates/candidates', (req, res) => {
        ...
        const sql = `
            SELECT r.id AS registration_id, r.user_id, r.application_no, r.status AS reg_status, r.form_data AS reg_form_data,
                   u.user_id_string, u.first_name, u.last_name, u.email,
                   o.id AS order_id, o.status AS order_status, o.amount,
            ...
            WHERE r.seminar_id = ?
              AND EXISTS (SELECT 1 FROM competition_submissions cs WHERE cs.user_id = r.user_id AND cs.seminar_id = r.seminar_id AND cs.category LIKE 'child_%')
            ORDER BY u.last_name, u.first_name
        `;
        db.all(sql, [seminarId], (err, rows) => {
            ...
            if (rows) {
                rows.forEach(row => {
                    let fd = {};
                    try { fd = JSON.parse(row.reg_form_data || '{}'); } catch(e) {}
                    if (fd.child_name && String(fd.child_name).trim() !== '') {
                        row.first_name = String(fd.child_name).replace(/\s+/g, ' ').trim();
                        row.last_name = ''; // clear so table doesn't print parent last name
                    }
                });
            }
            res.json(rows || []);
