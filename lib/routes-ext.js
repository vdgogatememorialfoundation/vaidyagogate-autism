...
const certKinds = require('./certificate-kinds');
...
module.exports = function registerExtendedRoutes(app, deps) {
    ...
    // ——— Certificate candidates (participant registrations, approved volunteers, or competition entries) ———
    app.get('/api/admin/certificates/candidates', (req, res) => {
        const seminarId = parseInt(req.query.seminarId, 10);
        const certType = certKinds.normalizeCertKind(req.query.certType);
        if (!Number.isInteger(seminarId) || seminarId < 1) {
            return res.status(400).json({ error: 'seminarId is required' });
        }
        if (certType === 'competition') {
            const compSql = `
                SELECT cs.id AS submission_id, cs.user_id, cs.application_no, cs.status AS reg_status,
                       cs.title AS competition_title, cs.category AS competition_category, cs.form_data AS competition_form_data,
                       r.id AS registration_id, r.application_no AS registration_application_no,
                       ...
                       cc.id AS certificate_id, IFNULL(cc.enabled, 0) AS cert_enabled,
                       0 AS scan_verified, cc.display_name,
                       'competition' AS cert_kind
                FROM competition_submissions cs
                JOIN users u ON u.id = cs.user_id
                JOIN seminars s ON s.id = cs.seminar_id
                LEFT JOIN registrations r ON r.user_id = cs.user_id AND r.seminar_id = cs.seminar_id
                LEFT JOIN orders o ON o.registration_id = r.id AND o.status = 'success'
                LEFT JOIN tickets t ON t.order_id = o.id
                LEFT JOIN competition_certificates cc ON cc.user_id = cs.user_id AND cc.seminar_id = cs.seminar_id
                WHERE cs.seminar_id = ?
                  AND cs.id = (
                      SELECT cs2.id FROM competition_submissions cs2
                      WHERE cs2.user_id = cs.user_id AND cs2.seminar_id = cs.seminar_id
                      ORDER BY CASE LOWER(COALESCE(cs2.status, 'submitted'))
                                   WHEN 'approved' THEN 0
                                   WHEN 'under_review' THEN 1
                                   WHEN 'submitted' THEN 2
                                   ELSE 9 END, cs2.id DESC
                      LIMIT 1
                  )
                ORDER BY u.last_name, u.first_name
            `;
            return db.all(compSql, [seminarId], (err, rows) => {
                if (err && /no such table|does not exist/i.test(String(err.message || ''))) return res.json([]);
                if (err) return res.status(500).json({ error: err.message });
                res.json(
                    (rows || []).map((r) => ({
                        ...r,
                        competition_category_label: certKinds.competitionCategoryLabel(r.competition_category),
                        competition_eligible: certKinds.competitionStatusEligible(r.reg_status) ? 1 : 0,
                        competition_recipient: certKinds.competitionRecipientName(
                            r.competition_form_data,
    ...
    app.post('/api/admin/certificates/bulk-toggle', (req, res) => {
        ...
        const type = certKinds.normalizeCertKind(certType);
                ...
                ids.forEach((uid) => {
                    db.get(
                        ...
                        (e0, regRow) => {
                            ...
                            if (type === 'competition') {
                                db.get(
                                    `SELECT cs.id AS submission_id, cs.application_no, cs.title, cs.category, cs.form_data, cs.status,
                                            u.user_id_string, u.first_name, u.last_name, 'competition' AS cert_kind
                                     FROM competition_submissions cs
                                     JOIN users u ON u.id = cs.user_id
                                     WHERE cs.user_id = ? AND cs.seminar_id = ?
                                     ORDER BY CASE LOWER(COALESCE(cs.status, 'submitted'))
                                                  WHEN 'approved' THEN 0
                                                  WHEN 'under_review' THEN 1
                                                  WHEN 'submitted' THEN 2
                                                  ELSE 9 END, cs.id DESC
                                     LIMIT 1`,
                                    [uid, sid],
                                    (eComp, compRow) => {
                                        if (eComp && !errMsg) errMsg = eComp.message;
                                        if (en === 1) {
                                            if (!compRow) {
                                                skipped.push({
                                                    userId: uid,
                                                    error: 'No competition entry on record for this event.'
                                                });
                                                done++;
                                                if (done === ids.length) finishBulk();
                                                return;
                                            }
                                            if (!certKinds.competitionStatusEligible(compRow.status)) {
                                                skipped.push({
                                                    userId: uid,
                                                    error: `Competition entry is ${String(compRow.status || '').replace(/_/g, ' ')} — certificate cannot be enabled.`
                                                });
                                                done++;
                                                if (done === ids.length) finishBulk();
                                                return;
                                            ...
                                            if (certVerify) {
                                                const chk = certVerify.validateCertMandatoryFields(compRow);
                                                if (!chk.ok) {
                            ...
                            function upsertCompetitionCert(compRow) {
                                const accountName = compRow
                                    ? [compRow.first_name, compRow.last_name].filter(Boolean).join(' ')
                                    : '';
                                const dn = compRow
                                    ? certKinds.competitionRecipientName(
                                          compRow.form_data,
                                          compRow.category,
                                          buildDisplayNameFromFormData(regRow && regRow.form_data, {}) || accountName
                                      )
                                    : buildDisplayNameFromFormData(regRow && regRow.form_data, {}) || 'Participant';
                                db.run(
                                    `INSERT INTO competition_certificates (user_id, seminar_id, submission_id, registration_id, display_name, template_id, enabled, updated_at)
                                     VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                                     ON CONFLICT(user_id, seminar_id) DO UPDATE SET
                                       enabled = excluded.enabled,
                                       submission_id = COALESCE(excluded.submission_id, competition_certificates.submission_id),
                                       display_name = COALESCE(excluded.display_name, competition_certificates.display_name),
                                       template_id = COALESCE(excluded.template_id, competition_certificates.template_id),
                                       updated_at = CURRENT_TIMESTAMP`,
                                    [
                                        uid,
                                        sid,
                                        compRow ? compRow.submission_id : null,
                                    ...
                                    (e) => {
                                        ...
                                        if (en === 1 && certVerify && !e) {
                                            db.get(
                                                `SELECT id FROM competition_certificates WHERE user_id = ? AND seminar_id = ?`,
                                                [uid, sid],
                                                (eCert, certRow) => {
                                                    if (eCert && !errMsg) errMsg = eCert.message;
                                                    if (certRow && certRow.id) {
                                                        certVerify.ensureCompetitionCertVerifyToken(db, certRow.id, () => {
                                                            done++;
                                                            if (done === ids.length) finishBulk();
                                                        });
                                                    } else {
        ...
        resolveInternalUserId(db, userId, userIdString || userId, (e, uid) => {
            ...
            if (!uid) {
                return res.status(404).json({
                    error: 'Applicant not found. Use portal User ID (letters and numbers, e.g. USR_...) or internal numeric id.'
                });
            }
            db.run(
                        ...
                        volunteerTicketFlow.ensureVolunteerRegistrationOverride(
                            ...
                            () => {
                                res.json({
                                    ...
                                    userId: uid,
                                    message:
                                        'Volunteer assigned. Applicant must complete seminar registration in the portal (or use Applicant applications tab). Free ticket (₹0) is issued only after registration is submitted.'
                                });
                            }
                        );
                    }
                ...
                (fErr, result) => {
                    ...
                    return res.status(400).json({
                        error:
                            ...
                            'Applicant must complete seminar registration before a free volunteer ticket can be issued.'
                    });
                }
            );
        });
    ...
    app.get('/api/doctor/competition-certificates/:userId', auxMw, (req, res) => {
        const uid = parseInt(req.params.userId, 10);
        if (!Number.isInteger(uid) || uid < 1) return res.status(400).json({ error: 'Invalid user id' });
        const sql = `SELECT cc.*, s.title AS seminar_title, ct.file_path AS template_path, ct.mime_type,
                            cs.title AS competition_title, cs.category AS competition_category, cs.application_no,
                            'competition' AS cert_kind
             FROM competition_certificates cc
             LEFT JOIN seminars s ON s.id = cc.seminar_id
             LEFT JOIN competition_submissions cs ON cs.id = cc.submission_id
             LEFT JOIN certificate_templates ct ON ct.id = cc.template_id AND COALESCE(ct.cert_type, 'participant') = 'competition'
             WHERE cc.user_id = ?`;
        db.all(sql, [uid], (e, rows) => {
            if (e && /no such table|does not exist/i.test(String(e.message || ''))) return res.json([]);
            if (e) return res.status(500).json({ error: e.message });
            res.json(
                (rows || []).map((r) => ({
                    ...r,
  ...
  function sendAdminReport(req, res) {
        ...
        db.all(spec.sql, [sid], (e, rows) => {
            ...
            if (format === 'pdf' || format === 'html') {
                db.get(`SELECT title FROM seminars WHERE id = ?`, [sid], (errSem, semRow) => {
                    const eventName = semRow ? semRow.title : `Event #${sid}`;
                    const html = exportReports.toHtmlTable(out, spec.title, eventName);
