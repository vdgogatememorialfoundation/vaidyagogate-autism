            ...
            db.get(`SELECT id FROM preregistrations WHERE application_no = ?`, [applicationNo], (err, existing) => {
                ...
                if (existing) {
                    ...
                    db.run(
                        ...
                        [firstName, lastName, email, phone, status, JSON.stringify(formData), applicationNo],
                        (updateErr) => {
                            if (updateErr) {
                                results.errors++;
                    ...
                    db.get(`SELECT id FROM users WHERE LOWER(TRIM(email)) = ? LIMIT 1`, [email], (userErr, existingUser) => {
                        ...
                        db.run(
                            `INSERT INTO preregistrations (user_id, seminar_id, application_no, first_name, last_name, email, phone, status, form_data, created_at, updated_at)
                             VALUES (
                                 (SELECT id FROM users WHERE LOWER(TRIM(email)) = ? LIMIT 1),
                                 ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now')
                             )`,
                            [email, sid, applicationNo, firstName, lastName, email, phone, status, JSON.stringify(formData)],
                            (preErr) => {
                                if (preErr) {
                                    if (preErr.message.includes('UNIQUE')) {
