...
function registerAutismPortalRoutes(app, deps) {
    ...
    app.post('/api/admin/users/send-all-welcome-emails', adminGuard, (req, res) => {
        db.all(
            `SELECT id, email, first_name, last_name FROM users WHERE is_disabled = 0 AND email IS NOT NULL AND trim(email) != '' AND password IS NOT NULL AND password != ''`,
            [],
            (err, rows) => {
                if (err) return res.status(500).json({ error: err.message });
                if (!rows.length) {
                ...
                rows.forEach((row) => {
                    // Generate new temporary password for this email
    ...
    app.post('/api/admin/users/send-welcome-to-selected', adminGuard, (req, res) => {
        ...
        db.all(
            `SELECT id, email, first_name, last_name FROM users WHERE id IN (${placeholders}) AND is_disabled = 0 AND email IS NOT NULL AND trim(email) != '' AND password IS NOT NULL AND password != ''`,
            ids,
            (err, rows) => {
                if (err) return res.status(500).json({ error: err.message });
                if (!rows.length) {
