...
function preregWindowState(seminar) {
    ...
    if (win.open) return { open: true };
    return { open: false, reason: win.reason || 'closed' };
}
                                ...
                                res.json({
                                    ...
                                    message: flow.autoAcceptPreregistration
                                        ? flow.mainRegistrationOpen
                                            ? 'Pre-registration accepted. You may proceed with main registration when it opens.'
                                            : 'Pre-registration accepted. Final registration will open later — we will notify you when it is available.'
