...
function registerAutismPortalRoutes(app, deps) {
    ...
    app.post('/api/admin/preregistrations/send-all-account-emails', adminGuard, (req, res) => {
        ...
        let sql = `SELECT p.id, p.user_id, p.seminar_id, p.application_no, p.status, p.email, p.first_name, p.last_name,
                          p.phone, u.id AS has_account, u.email AS user_email
                   FROM preregistrations p
                   LEFT JOIN users u ON u.id = p.user_id
        ...
        db.all(sql, params, (err, rows) => {
            ...
            rows.forEach((row) => {
                ...
                if (row.has_account) {
                    ...
                    const userIdStr = 'USR' + Date.now().toString(36).toUpperCase();
