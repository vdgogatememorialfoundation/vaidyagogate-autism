                ...
                if (existing) {
                    ...
                    db.get(`SELECT id FROM users WHERE LOWER(TRIM(email)) = ? LIMIT 1`, [email], (userErr, existingUser) => {
                        if (existingUser) {
                            ...
                            // Create new user - generate password but don't store it
                        ...
                        // Insert preregistration - use subquery to get user_id
                        ...
                        db.run(
                            ...
                            [email, sid, applicationNo, status, JSON.stringify(formData)],
                            (preErr) => {
                                if (preErr) {
                                    if (preErr.message && preErr.message.includes('duplicate') || preErr.message && preErr.message.includes('UNIQUE')) {
                                        results.preregs_updated++;
                                    } else {
                                        results.errors++;
