                    ...
                    db.get(`SELECT id FROM users WHERE LOWER(TRIM(email)) = ? LIMIT 1`, [email], (userErr, existingUser) => {
                            ...
                            db.run(
                                ...
                                (insertErr, result) => {
                                    if (insertErr) {
                                        ...
                                        // Get the inserted user id - result.rows[0].id for pg, result.lastID for sqlite
                                        userId = result && result.rows ? result.rows[0].id : (result && result.lastID);
                                    }
                                }
                            );
                        }
                        
                        // Insert preregistration - only use columns that exist in the table
                        ...
                        db.run(
                            `INSERT INTO preregistrations (user_id, seminar_id, application_no, status, form_data, created_at, updated_at)
                             VALUES (
                                 COALESCE((SELECT id FROM users WHERE LOWER(TRIM(email)) = ? LIMIT 1), ?),
                                 ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
                             )`,
                            [email, userId, sid, applicationNo, status, JSON.stringify(formData)],
                            (preErr) => {
                                if (preErr) {
                                    results.errors++;
