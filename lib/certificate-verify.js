...
function validateCertMandatoryFields(row) {
    const kind = certKinds.normalizeCertKind(row && row.cert_kind);
    ...
    if (!prn) {
        return {
            ok: false,
            error: volunteer
                ? 'Volunteer ID (Participant PRN) is missing for this certificate.'
                : 'Participant PRN is missing for this registration.'
        };
    }
    if (!applicationNo) {
        return {
            ...
            error: volunteer
                ...
                : competition
                  ? 'Competition entry ID is missing for this certificate.'
                  : 'Application No. is missing for this registration.'
        };
    }
    return { ok: true, prn, applicationNo, certKind: kind };
}

function certVerifyOtpPurpose(certId, certKind, channel) {
    const kind = certKinds.normalizeCertKind(certKind);
...
function ensureCertVerifyToken(db, certId, certKind, cb) {
    const kind = certKinds.normalizeCertKind(certKind);
    if (kind === 'volunteer') return ensureVolunteerCertVerifyToken(db, certId, cb);
    if (kind === 'competition') return ensureCompetitionCertVerifyToken(db, certId, cb);
    return ensureUserCertVerifyToken(db, certId, cb);
}

function ensureCompetitionCertVerifyToken(db, certId, cb) {
    const id = parseInt(certId, 10);
    if (!Number.isInteger(id) || id < 1) return cb(new Error('Invalid certificate id'));
    db.get(`SELECT id, verify_token FROM competition_certificates WHERE id = ?`, [id], (e, row) => {
        if (e) return cb(e);
        ...
        db.run(
            `UPDATE competition_certificates SET verify_token = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
            [tok, id],
            (e2) => {
                if (e2) return cb(e2);
                cb(null, tok);
...
function resolveCertForPublicLookup(db, { seminarId, applicationNo, prn, token, certKind }, cb) {
    ...
    if (tok) {
        ...
        const competitionSql = `
            SELECT cc.id AS cert_id, cc.user_id, cc.seminar_id, cc.enabled, cc.verify_token, cc.dispatched_at,
                   cc.display_name, 'competition' AS cert_kind,
                   cs.application_no, u.user_id_string, u.email, u.phone,
                   s.id AS sem_id, s.title AS seminar_title, s.event_date, s.certificate_verify_enabled,
                   s.certificate_verify_manual, s.certificate_verify_go_live_at
            FROM competition_certificates cc
            JOIN users u ON u.id = cc.user_id
            JOIN seminars s ON s.id = cc.seminar_id
            LEFT JOIN competition_submissions cs ON cs.id = cc.submission_id
            WHERE cc.verify_token = ? AND cc.enabled = 1
            LIMIT 1`;
        ...
        return db.get(participantSql, [tok], (e, row) => {
            ...
            db.get(volunteerSql, [tok], (e2, vrow) => {
                if (e2) return cb(e2);
                if (vrow) return runRow(vrow);
                db.get(competitionSql, [tok], (e3, crow) => {
                    if (e3 && !/no such table|does not exist/i.test(String(e3.message || ''))) return cb(e3);
                    runRow(crow || null);
                });
            });
        });
    ...
    db.get(
        ...
        (e, sem) => {
            ...
            afterSeminarGate(sem, (semRow) => {
                ...
                const competitionSql = `
                SELECT cc.id AS cert_id, cc.user_id, cc.enabled, cc.verify_token, cc.dispatched_at,
                       cc.display_name, 'competition' AS cert_kind,
                       cs.application_no, u.user_id_string, u.email, u.phone, s.title AS seminar_title
                FROM competition_certificates cc
                JOIN users u ON u.id = cc.user_id
                JOIN seminars s ON s.id = cc.seminar_id
                LEFT JOIN competition_submissions cs ON cs.id = cc.submission_id
                WHERE cc.seminar_id = ? AND cc.enabled = 1`;
                ...
                runSearch(participantSql, filterP, paramsP, (e2, pRows) => {
                    ...
                    runSearch(volunteerSql, filterV, paramsV, (e3, vRows) => {
                        if (e3) return cb(e3);
                        runSearch(competitionSql, filterC, paramsC, (e4, cRows) => {
                            if (e4 && !/no such table|does not exist/i.test(String(e4.message || ''))) return cb(e4);
                            let rows = (pRows || []).concat(vRows || [], cRows || []);
                            if (!rows.length) {
                                return cb(null, { ok: false, error: 'No matching issued certificate found.' });
                            }
                            const kindFilter = String(certKind || '').toLowerCase();
                            if (rows.length > 1 && certKinds.KINDS.includes(kindFilter)) {
                            ...
                            if (rows.length > 1) {
                                const kindsFound = rows.map((r) => certKinds.certKindLabel(r.cert_kind)).join(' and ');
                                return cb(null, {
                                    ok: false,
                                    error:
                                        kindsFound +
                                        ' certificates both match. Scan the QR on the certificate you are verifying, choose the certificate type, or enter its specific application / entry no.'
                                });
                            }
...
function dispatchCertList(db, seminarId, list, certKind, cb) {
    ...
    const nextRow = () => {
        ...
        ensureCertVerifyToken(db, row.cert_id, kind, (e3, tok) => {
            if (e3) {
                skipped++;
            ...
            const viewParam = certKinds.certViewParam(kind);
            const viewUrl =
                notifEngine.publicBaseUrl() +
                `/certificate/view?${viewParam}=${row.cert_id}&uid=${row.user_id}` +
                (kind === 'participant' ? '' : `&type=${kind}`);
            const certEventKey = row.dispatched_at ? 'CERTIFICATE_REISSUED' : 'CERTIFICATE_AVAILABLE';
            notifEngine.notifyUserEvent(
                ...
                () => {
                    const table = certKinds.certTable(kind);
                    db.run(
...
const COMPETITION_CERT_APP_NO_SQL = `cs.application_no`;

function listEnabledCertRows(db, sid, kind, cb) {
    const sqlByKind = {
        participant: `SELECT uc.id AS cert_id, uc.user_id, uc.registration_id, uc.dispatched_at, r.application_no, u.user_id_string, u.email, u.phone
                      FROM user_certificates uc
                      JOIN users u ON u.id = uc.user_id
                      LEFT JOIN registrations r ON r.id = uc.registration_id
                      WHERE uc.seminar_id = ? AND uc.enabled = 1`,
        volunteer: `SELECT vc.id AS cert_id, vc.user_id, vc.dispatched_at,
                    ...
                    LEFT JOIN registrations r ON r.id = vc.registration_id
                    WHERE vc.seminar_id = ? AND vc.enabled = 1`,
        competition: `SELECT cc.id AS cert_id, cc.user_id, cc.dispatched_at,
                             ${COMPETITION_CERT_APP_NO_SQL} AS application_no,
                             u.user_id_string, u.email, u.phone
                      FROM competition_certificates cc
                      JOIN users u ON u.id = cc.user_id
                      LEFT JOIN competition_submissions cs ON cs.id = cc.submission_id
    ...
    db.all(sqlByKind[kind], [sid], (e, rows) => {
        if (e && /no such table|does not exist/i.test(String(e.message || ''))) return cb(null, []);
        if (e) return cb(e);
        cb(null, rows || []);
    });
}
...
function dispatchAllEnabledCertificates(db, seminarId, cb) {
    ...
    db.get(`SELECT id, title, event_date FROM seminars WHERE id = ?`, [sid], (e, sem) => {
        if (e) return cb(e);
        if (!sem) return cb(null, { ok: false, error: 'Seminar not found' });
        const kinds = certKinds.KINDS.slice();
        const withTpl = [];
        const loadTpl = (idx) => {
            if (idx >= kinds.length) return afterTemplates();
            const kind = kinds[idx];
            const typeCond =
                kind === 'participant' ? `(cert_type IS NULL OR cert_type = 'participant')` : `cert_type = ?`;
            const params = kind === 'participant' ? [sid] : [sid, kind];
            db.get(
                `SELECT id, file_path FROM certificate_templates WHERE seminar_id = ? AND ${typeCond} AND is_active = 1 ORDER BY id DESC LIMIT 1`,
                params,
                (eTpl, tpl) => {
                    if (eTpl) return cb(eTpl);
                    if (tpl && tpl.file_path) withTpl.push(kind);
                    loadTpl(idx + 1);
                }
            );
        };
        const afterTemplates = () => {
            if (!withTpl.length) {
                return cb(null, {
                    ok: false,
                    error: 'Apply or upload participant, volunteer and/or competition certificate templates before dispatching.'
                });
            }
            const lists = [];
            const loadList = (idx) => {
                if (idx >= withTpl.length) return runDispatch();
                listEnabledCertRows(db, sid, withTpl[idx], (eL, rows) => {
                    if (eL) return cb(eL);
                    if (rows.length) lists.push({ kind: withTpl[idx], rows });
                    loadList(idx + 1);
                });
            };
            const runDispatch = () => {
                ...
                const step = (idx) => {
                    ...
                    dispatchCertList(db, sid, lists[idx].rows, lists[idx].kind, (eD, out) => {
                        if (eD) return cb(eD);
                        totalSent += out.dispatched;
                        totalSkipped += out.skipped;
                        allErrors.push.apply(allErrors, out.errors || []);
                        step(idx + 1);
                    });
                };
                step(0);
...
module.exports = {
    ...
    ensureCompetitionCertVerifyToken,
