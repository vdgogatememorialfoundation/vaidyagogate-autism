...
function orgFromEmail() {
    ...
    return (cfg && cfg.from) || process.env.ZEPTO_FROM || process.env.ZOHO_FROM || process.env.ADMIN_CONTACT_EMAIL || '';
}
...
function notifyParticipantByEmail(judge, participant, subject, bodyText, submissionId) {
    ...
    const html =
        ...
        portal +
        '">Applicant portal</a>, open <strong>Track case applications</strong>, view your application, and reply under <strong>Messages from judges</strong>.</p></div>';
    return emailSvc.sendEmail(to, subjWithRef, html, {
        text: bodyText + footer + '\n\nReply in the Applicant portal: ' + portal,
        fromDisplay: display,
        fromEmail,
