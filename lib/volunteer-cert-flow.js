...
function syncDualCertEligibilityFromTicketScan(db, certVerify, ticketId, buildDisplayNameFromFormData, cb) {
    ...
    db.get(q, [ticketId], (err, row) => {
        ...
        const paid =
            portalProduct.FEATURES.noFees || String(row.order_status || '').toLowerCase() === 'success';
        ...
        const autoEnableOnScan = !!(portalProduct.FEATURES.noFees && scanVerified);

        function upsertParticipantCert(pTpl) {
            const enabledOnInsert = autoEnableOnScan && pTpl ? 1 : 1;
            const enabledOnConflict = autoEnableOnScan && pTpl
                ? 'CASE WHEN excluded.scan_verified = 1 AND excluded.template_id IS NOT NULL THEN 1 WHEN user_certificates.enabled = 1 THEN 1 ELSE user_certificates.enabled END'
                : 'CASE WHEN user_certificates.enabled = 1 THEN 1 ELSE user_certificates.enabled END';
            db.run(
        ...
        if (autoEnableOnScan) {
            return ensureBuiltinTemplateIfMissing(db, row.seminar_id, 'participant', null, (eTpl, tid) => {
                if (eTpl) return cb && cb(eTpl);
                upsertParticipantCert(tid);
            });
        }
        getActiveTemplateId(db, row.seminar_id, 'participant', (e2, pTpl) => {
            if (e2) return cb && cb(e2);
            upsertParticipantCert(pTpl);
        });
    });
}
