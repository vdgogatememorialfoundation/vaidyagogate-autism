    ...
    db.get(
        ...
        (e, row) => {
            ...
            if (decision === 'reject_documents' && !reason) {
                return cb(null, { ok: false, error: 'Rejection reason is required so the applicant knows what to fix.' });
            }
                ...
                return cb(null, {
                    ...
                    error:
                        st === 'revision_required'
                            ? 'Waiting for the applicant to re-upload documents on this application.'
                ...
                (e2) => {
                    ...
                    cb(null, {
                        ...
                        message:
                            decision === 'approve'
                                ? portalProduct.FEATURES.noFees
                                    ? 'Application approved. Issue the e-ticket from Main registration when ready.'
                                    : 'Application approved. Applicant can proceed to payment.'
                                : decision === 'reject_documents'
                                  ? 'Applicant notified to re-upload documents on the same application number.'
                                  : decision === 'request_documents'
                                    ? 'Applicant notified to upload additional verification documents.'
                ...
                return cb(null, {
                    ...
                    error:
                        subSt === 'revision_required'
                            ? 'Waiting for the applicant to re-upload rejected files.'
                        ...
                        cb(null, {
                            ...
                            message:
                                decision === 'approve_for_judging'
                                    ...
                                    : decision === 'reject_documents'
                                      ? 'Applicant notified to re-upload on the same application ID.'
