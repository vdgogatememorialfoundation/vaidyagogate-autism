...
function doctorNotFoundMessage(hint) {
    ...
    if (/^\d{10,}$/.test(h)) {
        return 'That looks like an e-ticket number, not a portal User ID. Use the applicant PRN (e.g. USR_...).';
    }
    return 'Applicant not found. Enter portal User ID (e.g. USR_...) or internal numeric id.';
}
