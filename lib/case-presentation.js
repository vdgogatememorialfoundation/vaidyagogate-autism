...
function caseWindowState(program) {
    const now = Date.now();
    const startRaw = program.registration_start;
    const endRaw = program.registration_end;
    if (!startRaw || !String(startRaw).trim() || !endRaw || !String(endRaw).trim()) {
        return 'unscheduled';
    }
    const rs = seminarDt.parseSeminarMs(startRaw);
    const re = seminarDt.parseRegistrationEndMs(endRaw);
    if (rs == null || Number.isNaN(rs) || re == null || Number.isNaN(re)) {
        return 'unscheduled';
    }
    if (now < rs) return 'upcoming';
    if (now > re) return 'closed';
    return 'open';
}
...
function registerCasePresentationRoutes(app, deps) {
    ...
    app.post('/api/case/submit', (req, res) => {
        const handleSubmit = (req, res, files, r2UploadIds) => {
            ...
            if (!Number.isInteger(userId)) {
                return res.status(400).json({ error: 'userId required — please sign in again to the applicant portal' });
            }
                                        ...
                                        if (!totalFiles) {
                                            ...
                                            notifEngine.notifyUserEvent(db, 'CASE_PRESENTATION_SUBMITTED', {
                                                ...
                                                vars: {
                                                    ...
                                                    case_presentation_title: form.topic || ''
                                                }
                                            });
                                            return res.json({
                                                success: true,
                ...
                const runCapacityOrPriority = () => {
                    db.get(
                        ...
                        (ePri, priRow) => {
                            ...
                            if (win === 'unscheduled') {
                                return res.status(400).json({
                                    error: 'Application schedule is not set for this program yet'
                                });
                            }
                            if (win === 'upcoming') {
    ...
    app.post('/api/admin/case/programs/:programId/priority-invite', (req, res) => {
        ...
        if (!userRef) return res.status(400).json({ error: 'Applicant portal ID or email required' });
        ...
        resolveUser((eU, uid) => {
            ...
            if (!uid) return res.status(404).json({ error: 'Applicant not found' });
            casePriorityInvite.createPriorityInvitation(
                ...
                (err, out) => {
                    ...
                    res.json({
                        success: true,
                        message:
                            'Priority application created. Applicant will complete missing details in the portal (fast-track review).',
                        ...
                        db.run(
                            ...
                            () => {
                                ...
                                res.json({
                                    success: true,
                                    message:
                                        'Winner selected. Applicant already registered for seminar — only case track updated (seminar registration unchanged).'
                                });
                            }
                        );
                    } else {
                        db.run(
                            ...
                            () => {
                                ...
                                res.json({
                                    success: true,
                                    message:
                                        'Winner selected. Applicant may register for the linked seminar with priority (registration window override can be applied).'
                                });
                            }
                        );
                    }
                                    ...
                                    try {
                                        ...
                                        res.json({
                                            ...
                                            message: result.emailSent
                                                ...
                                                : 'Message saved. Participant can reply in the Applicant portal.',
