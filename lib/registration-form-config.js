...
function parseRegistrationFormPayload(raw) {
    ...
    const sections = stepSections.normalizeStepSections(
        parsed.stepSections,
        stepSections.DEFAULT_MAIN_REG_STEP_SECTIONS
    );
    return { fields, birthYearMin, birthYearMax, stepSections: sections };
}
...
function buildConfigPayload(fields, meta) {
    ...
    const out = {
        ...
        stepSections: stepSections.normalizeStepSections(
            m.stepSections,
            stepSections.DEFAULT_MAIN_REG_STEP_SECTIONS
        )
    };
