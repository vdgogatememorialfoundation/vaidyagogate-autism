...
const REPORT_QUERIES = {
    main_registration: {
        sql: `SELECT r.application_no, r.status, r.created_at, r.updated_at, r.form_data,
              u.user_id_string, u.first_name, u.middle_name, u.last_name, u.email, u.phone
              FROM registrations r
              JOIN users u ON u.id = r.user_id
              WHERE r.seminar_id = ?
              ORDER BY r.created_at DESC`,
        title: 'Main Registration list'
    },
    pending: {
    ...
    main_registrations: {
        sql: `SELECT r.id AS registration_id, r.application_no, r.status, r.created_at, r.form_data,
              u.user_id_string, u.first_name, u.middle_name, u.last_name, u.email, u.phone
              FROM registrations r
              JOIN users u ON u.id = r.user_id
              WHERE r.seminar_id = ?
              ORDER BY r.created_at DESC, u.last_name, u.first_name`,
        title: 'Main registration list'
    },
    all_participants: {
        sql: `SELECT r.application_no, r.status, r.created_at, r.form_data,
    ...
    check_in: {
        sql: `SELECT DISTINCT u.user_id_string, u.first_name, u.middle_name, u.last_name, u.email, u.phone,
              r.application_no, r.status AS registration_status, r.created_at AS registered_at,
              CASE WHEN COALESCE(t.is_scanned, 0) = 1 OR LOWER(r.status) = 'checked_in' THEN 'Checked in'
                   WHEN t.id IS NOT NULL THEN 'E-ticket issued (not arrived)'
                   ELSE 'Registered (no e-ticket)' END AS checkin_status,
              t.ticket_id_string, t.is_scanned,
              (SELECT COUNT(*) FROM ticket_scan_events e3
                WHERE e3.seminar_id = r.seminar_id AND e3.application_no = r.application_no
                  AND e3.outcome = 'success') AS scan_count,
              t.scan_time,
              (sc.first_name || ' ' || IFNULL(sc.last_name, '')) AS scanned_by,
              o.order_id_string, o.amount, o.status AS order_status, o.payment_date,
              r.form_data
              FROM registrations r
              JOIN users u ON u.id = r.user_id
              ...
              LEFT JOIN tickets t ON t.order_id = o.id
              LEFT JOIN ticket_scan_events se ON se.id = (
                  SELECT MAX(e2.id) FROM ticket_scan_events e2
                  WHERE e2.seminar_id = r.seminar_id AND e2.application_no = r.application_no AND e2.outcome = 'success')
              LEFT JOIN users sc ON sc.id = se.scanned_by
              WHERE r.seminar_id = ?
              ...
              ORDER BY (CASE WHEN COALESCE(t.is_scanned, 0) = 1 OR LOWER(r.status) = 'checked_in' THEN 0 ELSE 1 END), t.scan_time DESC, u.last_name, u.first_name`,
        title: 'Check-in report (all attendees: checked in, e-ticket issued, registered)',
        postFilter: (rows) =>
            rows.map((row) => {
                const pax = ticketAttendees.resolveAttendeesCount(row);
                return Object.assign({ attendees_on_pass: pax != null ? pax : 1 }, row);
            })
    },
    finance: {
