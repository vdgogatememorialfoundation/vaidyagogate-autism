...
/** Registration statuses that count as a submitted main (final) registration. */
const MAIN_REGISTRATION_STATUSES = [
    'submitted',
    'pending_approval',
    'revision_required',
    'documents_requested',
...
const REPORT_QUERIES = {
    ...
    main_registrations: {
        sql: `SELECT r.application_no AS main_registration_no, r.status AS main_registration_status,
              r.created_at AS main_registered_at, r.form_data,
              p.application_no AS preregistration_no, p.status AS preregistration_status,
              u.user_id_string, u.first_name, u.middle_name, u.last_name, u.email, u.phone,
              ...
              WHERE r.seminar_id = ?
                AND LOWER(COALESCE(r.status, '')) IN (${MAIN_REGISTRATION_STATUSES.map(() => '?').join(',')})
              ORDER BY r.created_at, u.last_name, u.first_name`,
        params: (seminarId) => [seminarId, ...MAIN_REGISTRATION_STATUSES],
        title: 'Main registrations'
    },
    all_participants: {
        sql: `SELECT r.application_no, r.status, r.created_at, r.form_data,
              u.user_id_string, u.first_name, u.middle_name, u.last_name, u.email, u.phone,
