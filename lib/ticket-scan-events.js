...
function listTicketScanEvents(db, seminarId, opts, cb) {
    ...
    let sql = `SELECT e.*, u.first_name AS scanner_first, u.last_name AS scanner_last,
                      r.form_data AS reg_form_data
               FROM ticket_scan_events e
               LEFT JOIN users u ON u.id = e.scanned_by
               LEFT JOIN registrations r ON r.application_no = e.application_no AND r.seminar_id = e.seminar_id
               WHERE e.seminar_id = ?`;
    ...
    db.all(sql, params, (err, rows) => {
        ...
        const list = (rows || []).map((r) => ({
            ...
            attendeesCount: ticketAttendees.resolveAttendeesCount({ form_data: r.reg_form_data }),
            outcome: r.outcome,
            message: r.message,
